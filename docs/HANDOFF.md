...
4. **Workspace／Portfolio 物理拆檔**（§18.7 定案＋**§18.7.1 執行盤點已確認，2026-06-30**）：app.js 11928 行。**新 session 照 §18.7.1 分 3 批做**：批1 `portfolio.js`（Portfolio.* 2822–3160，最乾淨）→ 批2 `workspace.js`（Workspace.* 6 ＋ 4 個工作台週曆 App.* 方法 3200–3780）→ 批3 `shared-render.js`（gantt/月曆，跳 kanban/report/predToWbsFormat 島，風險最高）。載入序 app.js 先、各檔 `?v=`、啟動點 DOMContentLoaded 在後。每批 node 錨點抽＋計數＋node --check＋160＋線上驗、獨立 commit。**Paul 將開新 session 執行。**
