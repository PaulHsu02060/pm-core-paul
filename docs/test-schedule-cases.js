...
const App = {};  // 同步複本 namespace stub（掛 App._reschedulePreview / App._computeSlack）
...
// ════ 塊3a 同步複本（app.js App._reschedulePreview + App._computeSlack，byte-identical；改 app.js 須同步此處）════
...
// ════ 塊3a：_reschedulePreview 方向分流映射 + _computeSlack 餘裕（期望值=外部 Excel 反向 WORKDAY，2026-01 日曆，週一~五無假日）════
// 直打 _reschedulePreview（映射 lateStart/lateFinish→plannedStart/plannedEnd）與 _computeSlack（餘裕/燈號），非只打引擎。
// C 群：backward 分流映射；S 群：interval 餘裕 + 三級燈號邊界（slack 5/0/負）+ 防並行鏈加總。
console.log('\n===== 塊3a. _reschedulePreview 分流映射 + _computeSlack 餘裕 =====');
if (typeof App._reschedulePreview !== 'function' || typeof App._computeSlack !== 'function') {
  console.log('[FAIL] 塊3a C/S 群待③：_reschedulePreview / _computeSlack 同步複本尚未加入測試檔（步驟③改 code 時補，補後自動轉綠）');
  fail++;
} else {
  const byWbs = (tasks, w) => tasks.find(t => String(t.wbs) === String(w));

  // ── C1 backward 單任務：同週反推映射 ──
  {
    const tasks = translatePreds([ mk({ wbs: '1', variant: 'vA', durationDays: 3 }) ]);
    const variants = [{ id: 'vA', name: '主案', schedule: { startDate: '', endDate: '2026-01-15', direction: 'backward' } }];
    App._reschedulePreview(tasks, variants, []);
    const A = byWbs(tasks, '1');
    check('C1 backward plannedStart=lateStart', A.plannedStart, '2026-01-13', 'dur3 end=01-15(四)→lateStart=addWorkdays(01-15,-2)=01-13(二)；映射 plannedStart=lateStart');
    check('C1 backward plannedEnd=lateFinish', A.plannedEnd, '2026-01-15', '末端 lateFinish=end 01-15；映射 plannedEnd=lateFinish');
  }

  // ── C2 backward 跨週末反推映射 ──
  {
    const tasks = translatePreds([ mk({ wbs: '1', variant: 'vA', durationDays: 3 }) ]);
    const variants = [{ id: 'vA', name: '主案', schedule: { startDate: '', endDate: '2026-01-19', direction: 'backward' } }];
    App._reschedulePreview(tasks, variants, []);
    const A = byWbs(tasks, '1');
    check('C2 backward 跨週末 plannedStart', A.plannedStart, '2026-01-15', 'end=01-19(一)→addWorkdays(01-19,-2)跨週末=01-15(四)');
    check('C2 backward 跨週末 plannedEnd', A.plannedEnd, '2026-01-19', 'plannedEnd=lateFinish=end 01-19');
  }

  // ── C3 backward 三任務鏈 A→B→C：逐節映射 ──
  {
    const tasks = translatePreds([
      mk({ wbs: '1', variant: 'vA', durationDays: 2 }),                    // A 源頭
      mk({ wbs: '2', variant: 'vA', predecessor: '1', durationDays: 2 }),  // B 中段
      mk({ wbs: '3', variant: 'vA', predecessor: '2', durationDays: 2 }),  // C 末端
    ]);
    const variants = [{ id: 'vA', name: '主案', schedule: { startDate: '', endDate: '2026-01-30', direction: 'backward' } }];
    App._reschedulePreview(tasks, variants, []);
    const A = byWbs(tasks, '1'), B = byWbs(tasks, '2'), C = byWbs(tasks, '3');
    check('C3 鏈 A plannedStart', A.plannedStart, '2026-01-23', '源頭 A.lateStart=01-23(五)');
    check('C3 鏈 A plannedEnd', A.plannedEnd, '2026-01-26', 'A.lateFinish=01-26(一)');
    check('C3 鏈 B plannedEnd', B.plannedEnd, '2026-01-28', 'B.lateFinish=01-28(三)');
    check('C3 鏈 C plannedStart/End', C.plannedStart + '/' + C.plannedEnd, '2026-01-29/2026-01-30', 'C 末端 dur2：start=01-29(四)/end=01-30(五)');
  }

  // ── S1 interval slack=5 → 綠燈（5 的下邊界，含等於 5）──
  {
    const out = computeScheduleBackward(translatePreds([ mk({ wbs: '1', durationDays: 3, targetEnd: '2026-01-14' }) ]));
    const s = App._computeSlack(out.results, '2026-01-05', '2026-01-14');
    check('S1 available', s.available, 8, 'WD(01-05一,01-14三)=8');
    check('S1 needed', s.needed, 3, '單任務 dur3→關鍵路徑3工作天');
    check('S1 slack', s.slack, 5, '可用8-需要3=5');
    check('S1 light=green', s.light, 'green', 'slack≥5→綠');
  }

  // ── S2 interval slack=4 → 黃燈 ──
  {
    const out = computeScheduleBackward(translatePreds([ mk({ wbs: '1', durationDays: 3, targetEnd: '2026-01-13' }) ]));
    const s = App._computeSlack(out.results, '2026-01-05', '2026-01-13');
    check('S2 available', s.available, 7, 'WD(01-05一,01-13二)=7');
    check('S2 slack', s.slack, 4, '7-3=4');
    check('S2 light=yellow', s.light, 'yellow', '0≤slack<5→黃');
  }

  // ── S3 interval slack=0 → 黃燈（0 的下邊界，≥0 且 <5）──
  {
    const out = computeScheduleBackward(translatePreds([ mk({ wbs: '1', durationDays: 3, targetEnd: '2026-01-07' }) ]));
    const s = App._computeSlack(out.results, '2026-01-05', '2026-01-07');
    check('S3 available', s.available, 3, 'WD(01-05一,01-07三)=3');
    check('S3 slack', s.slack, 0, '3-3=0');
    check('S3 light=yellow', s.light, 'yellow', 'slack=0→黃（邊界）');
  }

  // ── S4 interval slack=-1 → 紅燈 + earliestFinish/overDays ──
  {
    const out = computeScheduleBackward(translatePreds([ mk({ wbs: '1', durationDays: 3, targetEnd: '2026-01-06' }) ]));
    const s = App._computeSlack(out.results, '2026-01-05', '2026-01-06');
    check('S4 available', s.available, 2, 'WD(01-05一,01-06二)=2');
    check('S4 slack', s.slack, -1, '2-3=-1');
    check('S4 light=red', s.light, 'red', 'slack<0→紅');
    check('S4 earliestFinish', s.earliestFinish, '2026-01-07', '從start 01-05 順推 needed3：addWorkdays(01-05,2)=01-07(三)');
    check('S4 overDays', s.overDays, 1, 'WD(end 01-06,最快 01-07)-1=1工作天');
  }

  // ── S5 防並行鏈加總：needed 取最長鏈（6），非加總（9）──
  // 鏈甲 A(dur2)→B(dur4)、鏈乙 A(dur2)→C(dur1)，B/C 末端 targetEnd=01-30
  {
    const out = computeScheduleBackward(translatePreds([
      mk({ wbs: '1', durationDays: 2 }),                                            // A
      mk({ wbs: '2', predecessor: '1', durationDays: 4, targetEnd: '2026-01-30' }), // B 鏈甲末端
      mk({ wbs: '3', predecessor: '1', durationDays: 1, targetEnd: '2026-01-30' }), // C 鏈乙末端
    ]));
    const s = App._computeSlack(out.results, '2026-01-05', '2026-01-30');
    check('S5 needed=最長鏈6（非加總9）', s.needed, 6, 'minLateStart=A 01-23 / maxLateFinish=01-30 → WD=6；非 2+4+1+2=9');
  }

  // ── S5 映射：A.plannedStart（兩後續取 min）/ B.plannedEnd / C start=end（走 _reschedulePreview）──
  {
    const tasks = translatePreds([
      mk({ wbs: '1', variant: 'vA', durationDays: 2 }),                   // A
      mk({ wbs: '2', variant: 'vA', predecessor: '1', durationDays: 4 }), // B 鏈甲
      mk({ wbs: '3', variant: 'vA', predecessor: '1', durationDays: 1 }), // C 鏈乙
    ]);
    const variants = [{ id: 'vA', name: '主案', schedule: { startDate: '', endDate: '2026-01-30', direction: 'backward' } }];
    App._reschedulePreview(tasks, variants, []);
    const A = byWbs(tasks, '1'), B = byWbs(tasks, '2'), C = byWbs(tasks, '3');
    check('S5 映射 A.plannedStart', A.plannedStart, '2026-01-23', '兩後續取最早 min：經B=01-23 < 經C=01-28');
    check('S5 映射 B.plannedEnd', B.plannedEnd, '2026-01-30', 'B 鏈甲末端 dur4 lateFinish=01-30');
    check('S5 映射 C.plannedStart/End', C.plannedStart + '/' + C.plannedEnd, '2026-01-30/2026-01-30', 'C 末端 dur1：start=end=01-30');
  }
}
