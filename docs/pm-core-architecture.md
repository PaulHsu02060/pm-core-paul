...
- **Step 3（建立即落地）**：點「建立」→ 防呆彈窗確定 → 落地時寫 §19.2 專案欄＋**動態生成 PM 常駐協調任務**（`isPmCoord`，工期＝全案跨度、effort 依 S15/M20/L40，範本不含、`_stage2Commit` 生成），歸入 sidebar「設變案 / ECN」群組。**過渡**：戰情室 dashboard（§19.10 B）未施工前暫落一般專案內頁；dashboard 落地後改跳戰情室。dashboard 大表欄位＝任務/擔當/工期/**前置（白話三欄·不出現 FS，複用 `_predRowHtml`）**/**投入比例%**/預計結束/狀態·成因；**拿掉需交付欄**。前置未完成→下游催辦（硬鎖互動見 §19.8 洞7）。ECN＝簡化實體，無看板/甘特/月曆 view 工具列。
...
- ⓪ **選型引導頁 ✅ 已落地**（§19.10 A.0，`4c4bb51`＋`ad8a73f` 迭代：Modal XL＋subgrid 對齊＋hover 琥珀）① **開案畫面 ✅ 已落地**（§19.10 A.1 三列式定版，`ad8a73f`，多輪線上驗＋Gemini 覆核）② 設變專屬 dashboard（左 HUD＋右 3 頁籤，§19.10 B）——**下一件，先細節討論→Mockup 定版→施工** ③ BOM·ROI 差異四區（§19.6，收進 dashboard 頁籤 B）。施工準＝§19.1b/19.4/19.5/19.6/19.9。
...
**Phase 1 施工**：✅ sidebar「設變案／ECN」群組（`71c398c`）→ ✅ 統一入口＋選型引導頁（`4c4bb51`，§19.10 A.0）→ ✅ **ECN 開案畫面＋範本引擎（`ad8a73f`，§19.10 A/A.1 定版：三列式表單/S-M-L 即時反饋/防呆彈窗/單案制/琥珀主題/PM 常駐任務落地/§19.9 範本檔）**→ **下一件 戰情室 dashboard（§19.10 B）**：先細節討論→Mockup 定版→施工（HUD＋3 頁籤，大表前置走白話三欄、成因 Enum、迴圈列、PM 常駐列）；落地後開案改跳戰情室（現過渡落一般內頁）→ BOM·ROI（匯入舊/新→差異比對四區→切換方式/呆滯→目標成本→ROI→存資料不存檔 exceljs 重生）。
