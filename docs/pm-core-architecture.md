...
**B.2 戰情室 Mockup v2 定版（2026-07-02，Gemini 三輪覆核＋四地雷拆除全數達標；施工照此，覆蓋 B 原版面描述的對應項）**

- **滿版流動佈局（Fluid）**：拿掉置中卡片框——**HUD 固定 280px**（視覺定錨）＋右側工作區 `minmax(0,1fr)` 吃滿剩餘寬（頁面左右安全邊距 24–32px）。ECN 內頁＝高強度戰情監控，比照 Jira/Asana 進案內即滿版。
- **降學習成本四策略（拔掉 80% HintBox）**：
  1. **視覺化取代文字**：雙軌變異卡＝**進度條**（淺灰軌＋琥珀實心=實際%＋深色豎線=預期%＋中間紅虛線段標「落後 N%」），下方仍拆「執行落後（琥珀）/範圍蔓延（紅）」兩徽章。
  2. **白話微文案**：「在途工時」→**待消化工時**、「累計」→**歷史總投入**、「風險旗」→**專案異常變更統計**、蔓延/迴圈括號註「案子長大/被退重做」——字面自帶解釋。
  3. **空狀態教學**：計數=0 的旗標下一行淡字教學（如重啟：「結案後被重新開啟時，這裡會自動 +1」），有數據後功成身退。
  4. **操作即時預覽**：前置 Popover 內改銜接/緩衝 → 即時顯示「↳ 預計將使此任務延後 N 天開始（日期→日期）」，所見即所得。
- **ⓘ 只留兩顆**（進度卡＋大表投入%），深底反白氣泡＋ **0.2s hover 延遲**（防快速滑過閃爍）；「投入比例% 怎麼看」HintBox 收成大表標題旁 ⓘ；「＋插入任務」「甘特預覽」收進表格工具列一行。
- **表格四鐵則（Gemini 地雷拆除，施工必守）**：① `table-layout: fixed`＋各欄固定寬（任務欄 auto 吃剩餘）——長字串不撐破欄寬；② 任務名/前置欄超長 `nowrap+ellipsis` 截斷＋原生 `title` hover 全文；③ 數字欄（工期/投入%）**右對齊且 ✏ 圖示放數字左側**（個位數切齊同一條垂直線）、文字欄左對齊、列高 padding 上下 12px；④ **Popover 禁純 CSS 絕對定位**——必須帶視窗邊界碰撞偵測（靠右翻邊、靠底翻上）。
- **定案：進度不進大表逐任務並列**——總體健康看左 HUD、大表守「排程與資源操作」職責分離（資訊過載防呆）。
- 空狀態教學字最淡到 `--ink4`（WCAG AA）；「未開始」狀態標籤極淡灰底（`--bg`）非純白。
