...
**E. Mockup／落地狀態（2026-07-02 更新）**
- ⓪ **選型引導頁 ✅ 已落地**（§19.10 A.0，`4c4bb51`＋`ad8a73f` 迭代：Modal XL＋subgrid 對齊＋hover 琥珀）① **開案畫面 ✅ 已落地**（§19.10 A.1 三列式定版，`ad8a73f`＋修正批：教育卡綠框/上一步鏈/資料不保留警告/ECN 直落地）② **戰情室 dashboard Mockup v2 ✅ 定版**（2026-07-02，§19.10 B＋B.1＋B.2——**下一件＝施工**）③ BOM·ROI 差異四區（§19.6，收進 dashboard 頁籤 B，Tab A 施工完再開設計）。施工準＝§19.1b/19.4/19.5/19.6/19.9。
...
**Phase 1 施工**：✅ sidebar「設變案／ECN」群組（`71c398c`）→ ✅ 統一入口＋選型引導頁（`4c4bb51`，§19.10 A.0）→ ✅ **ECN 開案畫面＋範本引擎（`ad8a73f`，§19.10 A/A.1 定版：三列式表單/S-M-L 即時反饋/防呆彈窗/單案制/琥珀主題/PM 常駐任務落地/§19.9 範本檔）**→ ✅ 戰情室細節定案＋Mockup v2 定版（§19.10 B.1/B.2，2026-07-02）→ **下一件 戰情室 dashboard 施工（Tab A 任務與排程）**：`renderEcnDashboard` 分流（`openProject` 依 `ecnType`）＋滿版佈局＋HUD 四卡（含 baselineHours 凍結補 `_stage2Commit`）＋大表（fixed 佈局四鐵則/前置 Popover 邊界偵測/打回重測/插入任務=蔓延+成因窗/案內重排）＋事件時間軸頁籤 C 殼；落地後開案改跳戰情室（現過渡落一般內頁）→ 投入% 批量修改 → **全專案總覽「PM 跨案負荷」區塊**（§19.4 Phase 1 交付 b：PM 跨所有 active ECN 案常駐列日疊加→爆表紅燈；戰情室內不放導流連結、總覽的事在總覽看——Paul 定版）→ BOM·ROI（Tab B，施工前先設計：匯入舊/新→差異比對四區→切換方式/呆滯→目標成本→ROI→存資料不存檔 exceljs 重生）。
