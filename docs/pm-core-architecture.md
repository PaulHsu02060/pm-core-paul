...
**批 1：portfolio.js（最乾淨，零夾島）**
- 抽 `Portfolio.*` 全 21 方法：app.js **2822–約3160**（`buildTabsHtml`→`renderOverview`，含總覽算法 helper：`_live`/`_overdue`/`projectHealth`/`healthCounts`/`totalProgress`/`overdueTasks`/`choreRatio`/`_kpiSnap*`/`_trendBadge`/`currentStage`/`projectProgress`/`weekCapacity`/`deptLoad`/`weeklyTop`/`renderOverview`）。
- 其後 3183/3193 兩個 `App.*`（`buildProjectViewTabsHtml`/`buildReportTabsHtml`）**留 app.js**。
- 尾界＝`renderOverview` 的 `};`（node 錨點：起 `Portfolio.buildTabsHtml`、止 Portfolio 最後一個方法 `};`）。

**批 2：workspace.js**
- 抽 `Workspace.*` 6 方法 **＋ 4 個工作台週曆 App.* 方法**（一起搬）：`render`(3200)／`dashboardWeekShift`(3335)／`buildWeekScheduleHtml`(3340)／**`handleScheduleDragStart`(3583)／`handleScheduleDrop`(3593)／`pinTaskToWeek`(3628)／`unpinTaskFromWeek`(3640)**／`buildNextWeekTodoHtml`(3652)／`buildMemoListHtml`(3726)／`attachMemoDrag`(3741)。
- 那 4 個 App.*＝**週曆拖拉移動任務＋釘選/取消釘選本週**，100% 工作台功能、只是掛 App.*；跟著搬、**維持 App.* 名稱**（app.js 先宣告 App，照樣可賦值）→ 整段 **3200–約3780** 連續可抽。
- 尾界＝`attachMemoDrag` 的 `};`；其後 3782 `App.promptModal` **留 app.js**。

**批 3：shared-render.js（gantt/月曆，非連續、跳島，風險最高）**
- 抽 3 段、跳島：
  - 段A **8491–約8962**：gantt 全套（`renderGantt`/`buildGanttHeaderHtml`/`ganttShift`/`ganttToday`/`buildGanttFilterHtml`/`toggleGantt*`/`GANTT_STATUS_LABELS`/`GANTT_SOURCE_DESC`/`buildGanttRowHtml`/`_ganttPreds`/`_drawGanttLinks`）＋`renderMonth`。
  - 段B **9013–約9070**：月曆 picker（`buildYMPickerHtml`/`toggleYMPicker`/`monthShift`/`monthToday`/`monthYearShift`/`monthYearSelect`/`monthPick`）。
  - 段C **9544–約9580**：gantt 共用 helper（`_ganttDateRange`/`_ganttColumns`；亦被 Excel 匯出 `exportProjectWbs` 用）。
- **跳過的島（留 app.js）**：`renderKanban`(8963–9012)、`renderReport`＋週報 helper(9072–9528)、`predToWbsFormat`(9529–9543)。
- ⚠ 為什麼 renderGantt/renderMonth 不可搬進 Portfolio：被專案頁 gantt/月曆 tab 同時用（§12.1 單一真實來源），故留 top-level 共用層。
...
5. **務必先確認塊內無夾島**（像 PDCA 第一刀的 getProjectStages／批2 的 4 個排程方法）：抽前 `awk` 掃該區所有 `^(App\.|Workspace\.|Portfolio\.|function|const)` 定義，非目標者標明留或一起搬。

**未納入本輪（未來各自獨立批）**：`project.js`（Project 邏輯現散在 App.*，需先命名聚集 `Project.*` 再抽）；`report.js`（`renderReport` 區）；物件字面量命名空間（`D`/`U`/`Storage`/`CloudSync`/`Auth`，整塊自包含、最易抽但非當務）。
