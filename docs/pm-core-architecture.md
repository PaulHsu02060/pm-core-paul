...
- **專案層**：`ecnType:true`、`size:'S'|'M'|'L'`、`changeReason`（設變原因分類，Combo Box 可自訂）、`ecnNo`（結案必填，如 ECN-2026-0042）、`effectiveDate`（簽核生效日）、`status:'active'|'closed'|'reopened'`、`loopCount`、`reopenCount`、`epochs:[{v, frozenAt, snapshotHours}]`（版本快照，洞4）。
- **任務層**：`effortRatio`（投入比例%，預設由範本帶；非範本手動新增或既有任務 migration=100%）、`isLoopTask:true`＋`loopFromId`（迴圈派生來源；平表用連結表達「巢狀」，不改資料結構為樹）。
- **BOM 層**（掛專案）：面板 `bomBaseCurrency`／`bomQuoteCurrency`／`bomRate`（雙幣別＋匯率）；`bomRows:[{partNoA, replacePartNoB, oldPrice, oldQty, newPrice, newQty, approveStatus:'pending'|'approved'}]`（萬用 row，見 §19.6）。

### 19.3 開案流程（複用路線B + Stage 2）

- **Step 1（開案）＝複用既有「套用範本創建」_s1 頁**（勿自創）：選「設變案 S/M/L」範本＋填名稱/顏色/主案子案日期 → **排程模式由填哪個日期系統自判**（正推/倒推/精算，`_effScheduleDir`），畫「評估→備料→驗證→結案」＋甘特預覽/時程不足。**只加** 設變原因分類、`ecnNo`(可空)。詳見 §19.10 A。
...
### 19.6 BOM 成本差額工具（Phase 1＝純工具，內頁右側可收合抽屜）
...
- **單行價差 ＝ `newPrice×newQty − oldPrice×oldQty`**；底端「本次設變成本總差額 ＝ Σ 單行價差」（換基準幣後），掛全專案總覽 KPI（Cost Down/Up）。
...
- **Excel 智慧快貼/匯入**：貼 Excel 矩陣 → 自動對齊欄位解析成 rows（新做；既有貼上只到文字/截圖）；配 **`🗑清除重貼`** 按鈕（貼錯格式一鍵清空重來，防卡死）。
- **匯出買料清單**（複用 exceljs／`exportProjectWbs` 路徑）。
...
### 19.9 S/M/L 範本內容（定案 strawman，PM 開案後可增刪/調整）

> 分級標準＝**改不改模 ＋ 要不要重新認證**（比純看工期客觀）。比例＝該任務吃「負責人」一天的投入%（等待型低 10–20%、動手型高 50–80%、行政型 30–50%）。以下為系統開案帶出的預設骨架，PM 當下可勾選/增刪/調工期。

**S 碼（換料級，不改模免認證，約 10 工作天）**

| # | 任務 | 工期 | 比例% | 前置 | 需交付 | 階段 |
|---|---|---|---|---|---|---|
|1|設變需求分析|1|80|—||評估|
|2|影響評估(BOM/成本)|1|80|1|設變評估表|評估|
|3|替代料採購/索樣|3|20|2||備料|
|4|樣品確認|2|50|3||驗證|
|5|ECN簽核/BOM更新|2|50|4|ECN單|結案|
|6|量產放行|1|30|5|生效通知|結案|

**M 碼（結構/多料/需認定，約 25 工作天）**

| # | 任務 | 工期 | 比例% | 前置 | 需交付 | 階段 |
|---|---|---|---|---|---|---|
|1|設變需求分析|2|80|—||評估|
|2|影響評估(設計/BOM/成本)|2|80|1|設變評估表|評估|
|3|可行性確認|2|60|2||評估|
|4|新料採購/索樣|5|15|3||備料|
|5|部品認定|5|15|4|認定書|備料|
|6|樣品組裝|2|60|5||驗證|
|7|功能/安規測試|4|40|6|驗證報告|驗證|
|8|ECN簽核/BOM/文件更新|2|50|7|ECN單|結案|
|9|量產放行|1|30|8|生效通知|結案|

**L 碼（改模/認證/大改，約 55 工作天，含並行）**

| # | 任務 | 工期 | 比例% | 前置 | 需交付 | 階段 |
|---|---|---|---|---|---|---|
|1|設變需求分析|3|80|—||評估|
|2|影響評估(設計/BOM/成本/認證)|3|80|1|設變評估表|評估|
|3|可行性確認/立項|3|60|2||評估|
|4|改模發包|5|30|3||備料|
|5|開模→T0樣品|20|10|4|T0樣品|備料|
|6|部品認定|7|15|4|認定書|備料|
|7|樣品組裝|3|60|5,6||驗證|
|8|功能/安規測試|5|40|7|驗證報告|驗證|
|9|信賴性測試|10|15|7|信賴性報告|驗證|
|10|重新認證(安規/EMC)|15|15|8|認證證書|驗證|
|11|ECN簽核/BOM/文件更新|3|50|9,10|ECN單|結案|
|12|量產放行|2|30|11|生效通知|結案|
...
- **ECN 只加三處**：① 選擇範本下拉多「設變案 S/M/L」三筆（選了帶 ECN 階段 評估>備料>驗證>結案 與 §19.9 任務）② 設變原因分類（Combo Box 可選可填：Cost Down/設計錯誤/部品停產/法規變更＋自訂）③ 設變單號 `ecnNo`（開案可空、結案才必填）。子案＝多機種/規格。
