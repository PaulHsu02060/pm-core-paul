...
**A. 開案 Modal（畫面一，輕量但可展開）**
- **必填**：機型名稱/專案名稱（主鍵）、設變分級 S/M/L、基準日。
- **選填**：設變原因分類（**Combo Box 可選可填**：預設 Cost Down/設計錯誤/部品停產/法規變更，允許自訂如「客訴暫時對策」）；設變單號 `ecnNo`（**開案可空、結案才必填**，附提示語）。
- **時程排程模式切換**（複用三劍客 `_effScheduleDir`）：正推（從開始日計算）／逆推（從交期倒推）；依選擇動態出現「預計開始日」或「預計完工日」欄。
- **選 S/M/L 即在 Modal 內動態展開該範本 Task 清單 ＋ 小甘特預覽**：每行 Task 可勾選（要/不要）、改工期、`＋新增`/`🗑刪除`（Strawman：範本給預設、PM 開案當下擁最終剪裁權）。確認才「建立設變案」。
- 分工：Modal 展開＝**開案當下初步勾選/剪裁範圍**；建立後進內頁大表做**完整管理**（指派擔當、調比例、追狀態）。
