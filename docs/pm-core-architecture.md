...
- **專案層**（2026-07-02 開案畫面落地版 `ad8a73f`）：`ecnType:true`、`size:'S'|'M'|'L'`、`changeReason`（**設變背景與原因＝自由文字 textarea 整併欄**——原「原因分類 Combo」與「需求原因」合併，選填；原 datalist 分類與 Cost Down 自動推導已移除）、`roiType:'benefit'|'forced'`（效益型/被迫型，**純手動下拉**、不再由 changeReason 推導；§19.6）、`sourceNo`（**需求單號＝溯源**：觸發此案的客訴單/CAR/會議記錄編號，選填；往後與正式 ecnNo 關聯做開案↔結案追溯）、`ecnNo`（**開案一律空**——正式 ECN 單號結案打進 PLM/ERP 才產生，結案必填，如 ECN-2026-0042）、`effectiveDate`（簽核生效日）、`status:'active'|'closed'|'reopened'`、`loopCount`、`scopeGrowthCount`（範圍蔓延次數，§19.5 雙旗）、`reopenCount`、`epochs:[{v, frozenAt, snapshotHours}]`（版本快照，洞4）、`deliveryTracks`（交付軌：新機/重工包 retrofit kit，各自 BOM/SOP/結案，§19.6）、`stockSnapshot:{oldQtyLeft, consumeRate, runoutDate}`（生效日卡控快照，PMC 手填、系統算，§19.5）、`baselineHours`（開案落地當下凍結的總工時點數＝範圍蔓延分母，§19.10 B.1，dashboard 波實作）。
- **任務層**：`effortRatio`（投入比例%＝**六檔行為錨點 enum `0|10|25|50|75|100`**，選行為不填數字，§19.4；預設由範本帶並歸六檔；非範本手動新增或既有 NPI 任務 migration=100%；`0`＝掛名未出力舉證、非空值，§19.5）、`isPmCoord:true`（PM 常駐協調列標記，Model Y，§19.4）、`taskAttr:'baseline'|'conditional'|'fog'`（基礎常駐/條件常駐/迷霧衍生，§19.9）、`isLoopTask:true`＋`loopFromId`（迴圈派生來源；平表用連結表達「巢狀」，不改資料結構為樹）、`causeTag`（蔓延/迴圈事件成因 enum，Hybrid 強制確認，§19.5）、`predecessor`（**沿用既有前置結構 `{dep,type,lag}`，UI 走專案範本白話三欄、不顯 FS/SS**，§19.10 B）。（`expectedProgress`/`actualProgress`/`variance` 屬 runtime 動態算，不入範本 schema。）
...
### 19.3 開案流程（統一入口 → 選型引導頁 → 複用 _s1 表單 → 建立即落地，2026-07-02 定案）

- **統一入口（sidebar 入口重建）**：sidebar 兩群（開發專案／設變案 ECN）**上方**放單一「＋ 建立新案」主鈕；**移除**原本兩群尾各自的「＋新增專案」「＋新增設變案」鈕。兩個群組自此退化為**檢視清單（View）篩選器**，不再承擔開案功能。點「建立新案」→ 進**選型引導頁**（§19.10 A.0）。
- **Step 1（選型）＝選型引導頁**：兩欄（開發專案 NPI／設變案 ECN），頂部警示「開發↔設變的管理重點與介面完全不同、**立案後無法互轉**；S/M/L 規模開案後可隨進度調整」。選子卡（開發：套用範本／Excel 匯入／空白；設變：S／M／L）→ 進開案表單。**唯一不可逆界線＝開發↔設變**（內層與資料結構不同、不提供轉換 UI）；開發內 範本/空白/Excel 僅是帶任務方式（同一般專案內層）、設變內 S/M/L 為可調權重檔位。詳見 §19.10 A.0。
- **Step 2（開案表單）＝複用既有「套用範本創建」_s1 頁**（勿自創；ECN 版定版細節見 §19.10 A.1，2026-07-02 落地 `ad8a73f`）：ECN **跳過教育卡**直進 s1 頁（時程說明由開案小幫手 HintBox 承載）；填名稱/顏色/日期 → **排程列直接複用 _s1／`.s1-dynhint`**（日期、正逆推、餘裕同資料同邏輯，配色貼 ECN 琥珀，`_effScheduleDir`），**開始日/上市日擇一必填**（範本＋設變適用；空白/Excel 不強制日期）；畫階段骨架（S=3 段／M=6 段／L=7 段含 DR 審核，§19.9）＋甘特預覽/時程不足。**型別警示＝按「建立」時一次性防呆彈窗**（「此案將建立為工程設變案，建立後無法轉為一般開發案。確定？」——常駐型別 Banner 方案已廢，選型頁警示為第一道）。**ECN Phase 1＝單案制**（無子案/受影響機種入口，多機種留 Phase 2 與 variant 架構一起規劃）。
- **Step 3（建立即落地；2026-07-02 修訂：ECN 跳過 Stage 2）**：ECN 開案頁主鈕＝**「建立專案」直落地**（不進 Stage 2 檢視大表——任務調整本來就在戰情室大表 §19.10 B；紅燈時仍先導 `_ovf` 智慧面板）→ 防呆彈窗確定 → 落地時寫 §19.2 專案欄＋**動態生成 PM 常駐協調任務**（`isPmCoord`，工期＝全案跨度、effort 依 S15/M20/L40，範本不含、`_stage2Commit` 生成；`_commit` 內還原 topbar），歸入 sidebar「設變案 / ECN」群組。NPI 照舊「下一步：檢視任務」→ Stage 2 → 建立。**s1 頁「上一步」＝先彈資料不保留警告**（本頁輸入活在 DOM、重繪即失，NPI/ECN 都擋誤觸），確定才離開。**過渡**：戰情室 dashboard（§19.10 B）未施工前暫落一般專案內頁；dashboard 落地後改跳戰情室。dashboard 大表欄位＝任務/擔當/工期/**前置（白話三欄·不出現 FS，複用 `_predRowHtml`）**/**投入比例%**/預計結束/狀態·成因；**拿掉需交付欄**。前置未完成→下游催辦（硬鎖互動見 §19.8 洞7）。ECN＝簡化實體，無看板/甘特/月曆 view 工具列。
- **User 硬壓上市日 < 物理工期** → 觸發既有 `時程不足·超出 N 工作天` + 導 `_ovf*` 智慧面板（沿用 _s1 現行行為）。
...
**投入比例輸入＝六檔行為錨點下拉（2026-07-02 定案，取代自由填數字）**
- 病灶：叫 PM/DRI 自由填 % ＝抽象（「50%」是什麼？）＋主觀（同件事樂觀填 10、悲觀填 40）＋有摩擦（嫌煩就亂填）。**公平的操作型定義＝一致性**（同件事不管誰開、哪案，給同一數，且兩秒選得出），非會計級精準——這 % 唯一服務的決策是「同人同日 Σ>100% 爆表」，準到能判爆表就夠。
- 解法：**投入%欄改「篩選式下拉」、六個行為錨點，選行為不填數字**（行為比數字客觀、跨職務通用）。錨點量的是「這件事吃掉這個人一天多少」，**跟職務無關**，RD／採購／品保／業務／PM 同一套：

| % | 名稱 | 判準（共通，一句話能認） | 跨職務例子 |
|---|---|---|---|
| **0%** | 掛名未出力 | 列他名下、實際沒做（別人補上／擺爛）——**舉證，非刪除**（詳 §19.5） | 老油條掛備料卻沒動 |
| **10%** | 點一下就走 | 幾分鐘、一兩個觸點 | 簽核／回信／一通電話確認 |
| **25%** | 零星盯著跑 | 發出去等回來，一天回點幾趟，沒黏著 | 採購追料·追打樣／PM 追簽核 |
| **50%** | 半天在上面 | 佔掉半個工作天 | 跨部門會議·DR／品保喬承認條件 |
| **75%** | 大半天動手 | 這天主要在做它 | RD 改圖／DCC 彙整文件／現場試裝 |
| **100%** | 這天只有這件 | 獨佔 | 趕樣機／安規蹲整天／客戶談判當天 |

- **不留自訂逃生口**（Paul 定）：全系統 % 只有這六種值＝可比、可加總、爆表判斷乾淨；真有特例再議。
- **判準寫進介面（評判標準）**：①下拉選項自帶簡述（「🕐 零星盯著跑 · 25%」，非光禿數字）②大表上方投入比例 HintBox 列完整六檔判準表（§19.10 C）。
- **範本值歸六檔**：`ECN_TEMPLATE` 每條 effortRatio 依判準重歸六檔之一（§19.9；讓範本與下拉同一套值）。RD 定範本亦選六檔、不糾結填 55 還 60。
- **PM 常駐列例外**：其餘儲存格鎖死（§19.10 B.2），投入%欄**不套六檔下拉**（S15/M20/L40 不落六檔）→ 顯示該級預設、允許**向上微調**（≥預設，反映高複雜案 40%+ 重負載，Gemini 檢驗①），與六檔並行不衝突。
- **既有 NPI／手動 migration＝100% 不變**（不動 `Portfolio.deptLoad`，§19.1 (b)③）。
- **校準回路留 Phase 2**：第一版不求準、只求能爆表；跑幾個真實案後，結案時 PM 一鍵「這件其實比預估重／輕」→ 系統統計實際 vs 預估 → 建議校準範本預設。
...
**0% 掛名未出力 ＋ 代打/接手（護城河的第三種舉證，2026-07-02 定案）**
- **0% 的語意＝掛名但未出力，不是刪除、不是空白**：掛他名下、事實沒做（別人補上／擺爛）。配狀態欄分辨：`0%＋已完工`（別人收尾）＝該做沒做鐵證；`0%＋未開始`＝單純還沒輪到、不算帳。0% 天然自洽——工時點數＝0、對爆表加總貢獻 0，但留在負荷紀錄裡就是證據。
- **代打/接手＝雙列 pattern**（原負責人擺爛、PM 或他人接手實作）：**原條保留掛原負責人、投入%改 0%**（留「掛名沒做」證據）＋**插一條掛實際執行者的對應檔位**（前置接原條位置）。**禁用「直接改擔當」**——那會抹掉「原負責人掛名沒做」的證據，正好違反「同時記錄無所作為＋我的付出」的訴求。
- **Phase 1 輕量**：用既有「插入任務＋原條改 0%」＋備註／成因窗記代打原因，不新增資料結構。
- **正式化彙總留 Phase 2**：給「代打/接手」獨立事件類型，讓 HUD 直接彙總「N 條被代打／某 DRI 掛名 M 條零出力」＝考績／檢討客觀舉證（跟跨案負荷圖一起做）。
- **0% 列視覺**：大表 0% 列給淡標記（如淡灰「零投入」小標），檢討會一眼掃到。
...
### 19.9 設變流程瘦骨架 schema（2026-07-01 三方碰撞定案，取代原 S/M/L 胖範本；✅ 2026-07-02 落地 `templates/ecn-template.js` `ad8a73f`——e1–e13＋sizeMeta(S15/M20/L40·各級階段集)＋sizes 開關＋predBySize，oracle 驗三級前置零懸空；PM 常駐任務不入範本、落地時動態生成）
...
> 比例＝該任務吃「負責人」一天的投入%，**改六檔行為錨點下拉**（0／10／25／50／75／100，選行為不填數字，§19.4）；範本每條 effortRatio 依判準歸六檔之一；比例可加總 >100%＝爆訊號。以上為系統開案帶出的骨架，PM 當下可勾選/增刪/調工期/調比例。
...
**A.0 選型引導頁（Wizard，2026-07-02 定版，純白底＋俐落線框；統一入口在此接開案表單）**

> 解決「開案畫面＋內層畫面依分類完全不同、又不強制分類→ User 疑惑、隨便開了不能改重開」的問題：把型別選擇前置成一頁明確引導，選前就知道各分類的開案流程/內容/不可改界線。

- **入口（sidebar 入口重建，取代兩群尾＋鈕）**：sidebar 頂端唯一「＋ 建立新案」→ 整頁選型引導頁（比照 _s1 第一頁份量）。麵包屑「總覽表板 / 建立新案」→ 標題「建立新案 · 選擇類型」→ 副說明「先確認這是產品開發案，還是工程設變案——兩者的開案流程與內層畫面不同，選好類型再開始。」
- **視覺**（2026-07-02 落地版 `ad8a73f`）：全頁**純白底**、卡片**俐落細線框**（NPI 卡 `--sage-100` 淺綠邊·hover `--sage-400`／**ECN 卡預設中性淡框 `--rule`·hover 琥珀高亮 `--amber-accent`**——色點已足以識別，預設不搶視覺）、**無厚陰影**；群組色＝開發綠 `--sage`／設變琥珀 `--amber-accent`，對齊 sidebar 色點；分級色點 綠→琥珀→玫瑰（`--sage-500`／`--amber-accent`／`--rose`）＝輕→重。全色走 `:root`。
- **Modal 尺寸與對齊**（Gemini 覆核定版）：選型 Modal＝**XL 檔 `min(1150px, 88vw)`**（`.modal:has(.wiz)`，重大不可逆選擇給滿版專注；Design Tokens 見 UI 規範 §6），字級升舒適閱讀（卡標 14.5／內文 13.5／產出 12／欄標 15／警示 13.5px）；**左右兩欄用 CSS subgrid 共用 6 條 row 軌**（表頭/說明/卡×3/防過勞）→ 同 row 等高、左右卡片表頭全對齊，短卡在 row 內自然留白，卡距由 row-gap 統一。
- **頂部警示 banner**（淡沙底 `--hint-head`＋琥珀 ⚠）文案：「請注意：『新產品開發』與『工程設變』的管理重點與介面完全不同，立案後無法互轉，請依任務性質謹慎選擇。(註：設變案的 S/M/L 規模，開案後仍可隨進度調整。)」→ **唯一不可逆界線＝開發↔設變**（內層畫面＋資料結構不同、不提供轉換 UI）；開發內 範本/空白/Excel 僅是帶任務方式（同一般專案內層）、設變內 S/M/L 為可調權重檔位（§19.9 升降級）。
- **左欄 開發專案 · NPI**（綠 header）說明：「適合『從無到有』的新產品開發。提供標準甘特圖、WBS 與月曆等多種排程檢視模式。」三張卡（每卡＝標題＋內文＋底部「→ 產出」）：
  - **套用範本**：「載入公司標準開發流程，自動帶入各部門負責人與預設工期，最快建立完整骨架。」→ 產出：一般專案 Dashboard(含甘特／WBS)
  - **從 Excel 匯入**：「直接上傳您手邊的 WBS Excel 表格，系統會自動解析並轉為任務排程，無縫接軌。」→ 產出：一般專案 Dashboard(依 Excel 長甘特)
  - **空白專案**：「從零開始建立。無預設流程框架，適合階段特殊或不適用標準範本的微型專案。」→ 產出：一般專案 Dashboard(全空白自建)
- **右欄 設變案 · ECN**（琥珀 header）說明：「啟用設變專屬戰情室。精準追蹤『進度落差』、『重工次數』與『降本效益(ROI)』。」三張卡（色點 綠/琥珀/玫瑰）：
  - **S 級 · 輕量換料**：「適合單純的零件替代或文件修改。免安規重測，僅需走完『評估 → 改圖 → 結案』3 階段即可快速放行。」→ 產出：ECN 輕量戰情室(極簡 3 階段)
  - **M 級 · 結構認定**：「適合牽涉結構變更的中型案件。開啟標準 6 階段流程，包含實體打樣、品保測試與安規驗證等檢核節點。」→ 產出：ECN 標準戰情室(完整 6 階段)
  - **L 級 · 重大改模**：「適合牽涉改模或高風險的大型案件。除完整 6 階段外，強制追加跨部門『DR 設計審查』大關卡，嚴格卡控進度。」→ 產出：ECN 高規戰情室(含 DR 審查節點)
  - **L 級卡下方**綠字防過勞條（淺綠底 `--sage-50`＋`--sage-100` 邊）：「💡 防過勞機制：全級別皆強制掛載『PM 協調工時』以計算跨案負載。」**放 L 級下方（非主標下）＝讓左右兩欄三張卡片頂端對齊、排版對稱。**
- **選子卡 → 進 §19.10 A/A.1 開案表單**：開發子卡走 _s1 三種既有帶任務方式（範本/Excel/空白）；**設變子卡 onclick＝`_flowStartEcn(size)`**——設 ECN 模式（`App._s1Ecn={size}`）、預載名冊角色（ECN_TEMPLATE.roles）、**跳過教育卡**直進 s1 頁。**空白/Excel 不強制日期**（見 A.1 日期規則）。

**A. 開案畫面（直接複用既有「套用範本創建」_s1 第一階段預覽頁，勿自創 Modal；2026-07-02 落地 `ad8a73f`）**
- **既有現成、直接沿用**（template.js `_flowStartEcn`／`_deptEditorHtml`／`_s1*`）：專案名稱／辨識顏色（色點補 `_s1PickColor` 點擊切換）／日期卡（開始日/上市日期）／「階段區間預覽」甘特＋時程不足三色燈。**「上一步」走 `_s1Back`**（還原 topbar＋切回原頁再開選型 modal，背景不殘留）。
- **排程模式＝系統自判、不做手動 toggle**（沿用現行 UX，使用者只選時間）：只填開始日→正推、只填上市日→倒推、兩填→雙向精算餘裕（`_effScheduleDir`／`_s1ComputePreview`）。
- **範本引擎接線**：`ECN_TEMPLATE`（templates/ecn-template.js，§19.9 落地）→ `_ecnTplForSize(size)` 依 S/M/L 派生子範本（過濾 `sizes` 不含該級任務＋解析 `predBySize` 覆寫前置，前置零懸空、applyTemplate 零改動可吃）→ `_s1Tpl()` 單一取用點（ECN 模式回派生範本，否則回 PRODUCT_DEV_TEMPLATE）。`applyTemplate` additive 帶過 `effortRatio`/`taskAttr`（NPI 預設 100/baseline）。
- **部門/負責人名冊提前到 Stage 1 填**（複用既有 dept component `buildDeptRowsHtml`／`deptUI`，位置在階段區間預覽之後、頁尾前）：Stage 1 填好名冊 → 套範本依「部門→人」自動帶擔當（`applyTemplate`）→ 內頁大表「擔當」欄＝**下拉選名冊**（可改）。名冊區帶「為什麼要先填名冊？」HintBox（**預設展開**——Paul 定版，讓人一眼知道要填什麼）：①自動帶擔當 ②大表下拉直選 ③負荷自動計算·跨案同人同日 >100% 紅旗。
- 範本任務的勾選/工期微調沿用既有 _s1/_s2 能力；建立後進內頁（B）指派擔當/調比例/追進度。**不另做 Modal、不自創排程模式選擇。**

**A.1 開案畫面定版規格（2026-07-02 落地 `ad8a73f`，經多輪線上驗＋Gemini 覆核；往後照此維護）**

- **版面（上而下·三列式「全寬→雙欄→全寬」交錯，Gemini 定稿）**：麵包屑「總儀表板 / 新增設變案」→ 頁標題「套用範本創建 · 設變案」→ 副說明「點 S/M/L 看時程與 PM 負荷即時變化。排程模式由你填的日期系統自判。」→ 專案名稱＋辨識色點（`_s1PickColor` 可點切換）→ **開案小幫手 HintBox(A.2，收合)** → **Row1 設變分級全寬**〔S/M/L 三按鈕＋正下方**固定高度(min-height 40px)淡琥珀提示塊** `#s1-sz-hint`（點誰顯示誰的白話說明＋PM Effort%，文字長短不跳版）〕→ **Row2 不對稱雙欄(40/60)**〔設變類型下拉＋下方淡琥珀動態 hint `#s1-roi-hint`｜需求單號(選填)＋**? hover 氣泡**〕→ **Row3 全寬 textarea「設變背景與原因」**（3 行，**選填**，placeholder「請描述變更背景，例：客戶反映壓縮機外殼異音，需增加緩衝墊片」）→ 日期卡（單案制：**無主案膠囊/案名欄/階段膠囊列**，上方專案名稱＝唯一值）＋排程建議行 → 階段區間預覽甘特(帶日期＋PM 協調條) → 部門/負責人名冊(HintBox 預設展開) → Footer〔上一步(`_s1Back`)／下一步·檢視任務〕。
- **主題（`.s1-ecnpage` scope）**：`.s1-preview` 白底卡容器（NPI/ECN 共用，內外層一致）；ECN 操作元件全琥珀——S/M/L 選中態、兩處提示塊、排程建議行（含初始引導態）、PM 協調條、footer 主鈕；**紅色警示態與甘特三色燈不動**（狀態語意）；**左側 4px 粗裝飾線拿掉**（單案制無縮排視覺）。NPI 維持 sage。
- **S/M/L 動態分級即時反饋（核心互動，`_s1SetEcnSize`）**：點分級 → 各案階段集重設為該級 sizeMeta＋甘特＋PM Effort%＋排程建議＋分級說明行 即時重繪（**不整頁重繪，已填日期不掉**）。S＝3 階段(立案評估→設計變更→生效結案)、PM 15%；M＝6 階段、PM 20%；L＝7 階段（+「DR 審核」）、PM 40%。對應 §19.9「S/M/L＝條件任務開關＋工期檔位」；升級 Append、降級 PM 手動修剪。
- **設變類型（ROI）＝純手動下拉**（效益型/被迫型；原「原因選 Cost Down 自動推導」隨原因欄整併移除）：切換→下方動態 hint 跟著換——效益型「系統將於結案時比對新舊 BOM 表，自動結算降本效益（ROI）」／被迫型「系統將著重追蹤舊料消耗率，協助卡控生效日以降低報廢損失」。
- **需求單號（溯源，`sourceNo`）**：? 圓標 hover CSS 氣泡（原生 title 不穩）「此單號往後會與正式 ECN 單號關聯，記錄在專案內頁中，作為開案與結案的追溯紀錄。」placeholder「觸發此案的客訴單或異常單號」。
- **建立前防呆彈窗（取代常駐型別 Banner，Gemini 覆核定版）**：按「建立」→ `confirmModal`「此案將建立為工程設變案（ECN），建立後無法轉為一般開發案。（S/M/L 分級之後仍可調整）確定建立？」確定才進既有落地鏈；選型頁警示＝第一道、此彈窗＝最後閘門。**NPI 無 Banner 無彈窗**（預設流程）。
- **Footer（2026-07-02 修訂）**：ECN 主鈕＝**「建立專案」直落地**（跳過 Stage 2，見 §19.3 Step 3）；「上一步」＝先彈**資料不保留警告**（確定才回上一動：NPI→教育卡→選型頁、ECN→選型頁）。
- **PM 協調負荷條（Model Y 視覺化·招牌）**：甘特最下方固定一條「PM 協調 · 常駐·全程鎖定 · Effort N%」淡琥珀虛線條，每案強制、不可降級（S15→M20→L40）；**落地時生成真任務**（見 §19.3 Step 3）。
- **排程建議行（複用 `.s1-dynhint`，同資料同邏輯不另做）**：正推/逆推/雙向精算文案沿用；超出死線→紅警示（alert 態保留 rose）。**日期必填規則：開始日/上市日擇一必填或兩填（範本＋設變案適用）；空白/Excel 不強制日期**。
- **暴走／時程不足告警**：走既有 `_ovf` 三色燈那套，**不做閃爍**（a11y）。
- **Phase 1 拿掉（留 Phase 2 與 variant 架構一起規劃）**：受影響機種 Fan-out／新增子案入口／階段膠囊增刪改名（ECN 階段由 S/M/L 決定；NPI 案卡膠囊列照舊）。
...
- 展開 ol 四條（友善文案定版，2026-07-02 落地）：① 分級 S/M/L 判準（S 換料免認證／M 結構需認定／L 改模）＋執行中範圍擴大自動提醒升級；② 原因分類：效益型結案自動對比新舊 BOM 算省下成本、被迫型追舊料消耗率卡生效日降報廢；③ 時程推算：填開始日順推完工／只填上市日回推最晚動工／超死線亮紅；④ PM 協調工時（防呆保護）：每單預設保留 PM 跨部門協調時間、多案疊加總負載 >100% 發排擠告警防隱形過勞。
- **小尾巴（下波順手）**：②條開頭「原因分類」與現行 UI 名詞「設變類型」不一致（原因欄已整併），dashboard 施工時順手改文案。

**B. 設變專屬 dashboard（畫面二；2026-07-01 Mockup 定版，複用專案範本 Stage 2 Component ＋ ECN 專屬層；✅ 2026-07-02 Tab A 全落地並上線 `6aae2e0`）**

> **設計原則（2026-07-02 定，跨此模組通用鐵則）：**
> 1. **計畫 vs 實際分離**：大表「日期」欄＝**計畫排程**（planned，工期/前置/錨點驅動，餵預期進度）；**實際開工/完工日另存**（在編輯彈窗記錄）。兩者不混。
> 2. **狀態＝衍生·不手選·實際優先**（`_ecnStatusDerive`）：實際完工→已完工＞擱置＞實際開工→進行中＞計畫結束已過(無實際)→逾期＞計畫已到開工日→進行中＞未開始。**時間到 ≠ 做完**（系統只知時間、不知你做完沒）；「完成」一定靠**人工記錄實際完工日**（呼應系統 §五、getEffectiveSchedule actual>planned）。逾期＝計畫結束過了但沒實際完工，是警訊。
> 3. **外層/內層分層心智模型**：**外層大表＝排程規劃＋狀態監控**（PM 綜觀全局、處理相依/資源）——只擺「列間＋插入(中途追加)」與「⚙編輯」入口，**外層不放任何改狀態的快捷鍵**；**內層彈窗＝進度回報＋異常處置**（實際開/完工日·擱置+原因·打回重測·刪除，強制記成因保全客觀證據）。
> 4. **一鍵開工/完工快捷 暫不做**（Paul 2026-07-02 拍板）：進行中已由時間自動衍生、無需手動開工；完工是里程碑且日期要準（一鍵押今天會登錯、洗白逾期），維持「一律進彈窗填實際日期」。實測若補登太頻繁，再加「✓完工」折衷（跳彈窗確認日期，非無腦押今天）。
>
> **ECN＝簡化實體**：不套用一般專案的 view 工具列（**無看板/甘特/月曆 切換鈕**），就是這個 dashboard 三頁籤。**開案（A）建立即落地、自動跳此頁**（`renderProject` 依 `ecnType` 分流 `renderEcnDashboard`）。Sidebar **比照既有「NPI 開發案」格線群組，新增「設變案 / ECN」群組**（複用同一 sidebar 分組 component），下方列各 ECN 案，點進＝回此 dashboard 編輯。
...
- **右側 3 資料夾頁籤**（選中白底與內容連體、未選淺灰底＋邊框）：
  - **A 任務與排程（預設）**：投入比例 HintBox（淺色收合，C）→ 甘特（**收合、預設收起**）→ **大表**。
    ...
    - **日期欄＝計畫排程**（planned；工期/前置/開始錨點驅動，餵預期進度）。**狀態＝唯讀·衍生·實際優先**（`_ecnStatusDerive`）：實際完工日→已完工＞擱置＞實際開工日→進行中＞計畫結束已過(無實際)→逾期＞計畫已到開工日→進行中＞未開始。跟系統 §五/getEffectiveSchedule「actual>planned」同精神。**逾期＝計畫結束日過了但沒實際完工**（時間到≠做完，完成須人工記錄）。
    - **實際進度＋特殊狀態在「⚙ 編輯」modal 記錄**（`_ecnEditTask`）：**實際開工日／實際完工日**（填完工日＝已完工·退待消化工時·`progress=100`；只開工日＝進行中；皆空＝依計畫衍生）／**擱置**(checkbox＋原因 `holdReason`)／**打回重測**(有實際完工日才有→成因窗→`loopCount++`＋生成 `isLoopTask` 重做列，`_ecnLoopTask`)／刪除(二次確認)。實際日期不改計畫排程（computeSchedule 讀 `t.start`/前置，不讀 actual），無排程副作用。
  - **B BOM · ROI**：整套見 §19.6（匯入舊/新→差異比對→四區→切換方式/呆滯→目標成本→ROI→存資料不存檔）。
  - **C 版本歷史**：v1→v2 波折時間軸，每節點標成因（供應商 Fail 迴圈+1、客戶追加蔓延+2）→ 結案求償舉證。

**B.1 戰情室施工細節定案（2026-07-02，Claude 6 點 × Gemini 覆核全 OK＋UI 微調兩條）**

1. **雙軌變異計算口徑**：預期進度＝以「今天」對照排程，每任務 `已過工作天/工期`（cap 100%）**工時點數加權**彙總；實際進度＝複用 `taskDisplayProgress` 工時點數加權；執行落後＝實際−預期（負值琥珀）；範圍蔓延＝(當前總工時點數−baseline)/baseline（紅）。**開案落地當下凍結 `baselineHours`**（§19.2 專案層補欄）。
2. **雙旗互動入口**：蔓延🌫＝大表「插入任務」→ 彈成因確認窗（enum 下拉＋備註）→ `scopeGrowthCount++`；迴圈🔄＝**已完工任務列**「↩ 打回重測」鈕 → 派生 `isLoopTask` 紅底列（工期/比例帶原值可改）＋成因窗 → `loopCount++`。
3. **大表編輯後重排**：改工期/前置 → **案內自動重排**，複用 Stage 2 `_s2SetDuration`/`_reschedulePreview` 模式（依賴驅動順推、手動釘錨不覆蓋）；**不碰全域智慧排程**（ECN 任務不進工作台時段排程）。
4. **頁籤 C 版本歷史 Phase 1 範圍**＝**事件時間軸**（開案/蔓延+1/迴圈+1/分級變更，每筆帶日期＋成因）；epoch 凍結/v1 唯讀＝結案功能一起（下波）。
5. **結案流程**（ecnNo＋生效日必填→closed→工時凍結＋生效日雙卡）＝**不在這波**，dashboard 基礎＋BOM 後接著做。
6. **S/M/L 升降級入口**＝頂部分級徽章點擊 → 確認窗（升級＝Append 該級新增條件任務＋記事件；降級＝只改標記、任務 PM 手動修剪，照 §19.9）。
- **附帶定案**：工時 HUD 口徑＝在途（未完工任務點數 Σ）／累計（全任務點數，Phase 1 單 epoch）；甘特預覽＝複用 s1 那套唯讀收合；**投入% 批量修改＝大表落地後同波尾接著做**。
- **Gemini UI 微調（定案）**：① **前置白話三欄不放大表底部固定區塊**——hover／點擊該列「前置」欄 → **Popover（氣泡對話框）或行內展開**顯示三欄，視線聚焦（Fitts's Law）；② **戰情室主色調/狀態標籤/PM 協調列全琥珀**（暖森林綠 sage 只留 NPI 專案）；③ PM 常駐列設計維持。

**B.2 戰情室 Mockup 終版（2026-07-02 v7，經 Paul×Gemini 七輪迭代定版；施工照此，覆蓋 B 原版面描述的對應項）**

- **滿版流動佈局（Fluid）**：拿掉置中卡片框——**HUD 固定 280px**（視覺定錨）＋右側工作區 `minmax(0,1fr)` 吃滿剩餘寬（頁面左右安全邊距 24–32px）。ECN 內頁＝高強度戰情監控，進案內即滿版。
- **HUD 四卡（白話名詞定版）**：
  1. **工時**：「待消化工時」（未結案任務時數 Σ，完工自動扣除）＋「歷史總投入」（含重工/退件全部投入）。
  2. **異常變更統計**：「📌 中途追加任務（案子被加大）」「↩ 完工後被退重做」「↻ 結案後翻案重啟」三計數（紅字）＋計數=0 顯示一行淡字空狀態教學（`--ink4`，WCAG）＋「異常成因：客戶方 X%｜供應商 Y%」彙總列。
  3. **進度：目標 vs 實際**＝**進度條視覺化**（淺灰軌＋琥珀實心=實際%＋深色豎線=預期%＋紅虛線段標「落後 N%」）＋下方兩徽章「真的做慢（琥珀）/案子變大被稀釋（紅）」。
  4. **部門/負責人名冊**（純顯示；「PM 跨案總負荷」導流連結**移除**——總覽的事去總覽看，見 §19.11 待辦）。
- **說明策略（Paul 定版：內層空間夠，說明回歸 HintBox、不靠 hover ⓘ/tooltip）**：左 HUD 頂「💡 戰情指標面板說明」**預設展開**；大表上方「💡 任務大表操作指南」**預設收合**（垂直空間留給任務列表）。文案定稿（Gemini 潤）：
  - 面板說明三條：①工時水位（待消化=本版尚未結案任務時數、完工即自動扣除；歷史總投入=含重工退件的所有沉沒心血）②防彈衣機制（記錄中途追加/退回重做/結案翻案，每筆強制綁成因＝結案檢討與進度延遲的客觀舉證）③雙軌進度對比（豎線=預期、實心=實際，自動精算落差、釐清是執行落後還是範圍蔓延）。
  - 操作指南四條：①負荷警示燈（投入%評估單人單日負荷、無須湊滿 100%，跨案疊加同人單日 >100% 亮紅燈）②行內速改與權限（點儲存格直接編輯任務/擔當/工期/前置；PM 全局、擔當僅限自身）③智慧排程連動（前置全白話；改前置/工期自動重算時程，手動鎖定日期不在此限）④變更觸發點（列間「＋插入任務」或操作欄「⚙ 編輯→↩ 打回重測」→ 強制成因窗＋連動左側統計）。
- **大表＝複用 Stage 2 真實結構（`_s2ListHtml`／`.s2-tbl`／`_s2PredCells`），琥珀化＋欄位調整（2026-07-02 資料源決策：共用不重寫）**：
  - **資料源參數化（Paul 定，不另寫 `_ecnListHtml`）**：`_s2ListHtml` 與其編輯 handler（`_s2SetDuration`／`_s2SetPred`／`_s2SetOwner` 等）現讀寫 `_tplPreview`（建立前 preview）；戰情室讀寫 `DATA.tasks`（已落地）。**同一組函式加「資料來源」參數/mode 分流（preview↔DATA.tasks＋落地即存檔），不複製第二份 render**——設計準則：不重複、Table 不肥大。琥珀化走 CSS scope（`.s2-tbl` 在 ECN 容器內套琥珀變數），非另寫樣式。
  - 欄序：序(38)/任務名(228，inline input＋關鍵路徑 tag)/**部門(下拉·可換部門)**/擔當(下拉名冊)/**前置三窄格(序號 select 64＋銜接方式 select 126＋緩衝 number 48——Stage 2 現成 inline 模式，Popover 方案廢)**/工期(number)/**投入%(六檔行為錨點 select，佔原「需交付」欄位，§19.4——非自由 number；Mockup 顯示的 number input 為六檔定案前舊版)**/日期起訖(**開始/結束兩個 date input 可手動改，§6.5 開始為錨；直向堆疊省寬**)/狀態·成因(126)/**操作(96)**。
  - **部門欄可編輯（2026-07-02 定案，設變 vs NPI 思維差異）**：NPI 範本＝公司流程定死「這項就是該部門做」→ 部門固定、只換擔當（人）；**ECN 設變＝任務由哪個部門扛不固定**（臨時換部門＋擔當來做很常見）→ **部門欄改下拉可換**（選名冊部門名），換部門時 `t.role`＋`t.dept` 同步更新、若原擔當不屬新部門則清空重選、擔當下拉即時更新為新部門成員優先（`_ecnSetDept`/`_ecnDeptOptions`）。PM 常駐列部門仍鎖死。
  - **排程口徑＝forward（開始為錨）**：ECN 為已建立 live 專案，`_effScheduleDir` 在「開始+結束都有」時強制回 interval（反推），故落地/進戰情室時**清 endDate（改存 `targetEndDate`）轉 forward**（`_ecnForwardVariants`）→ 改工期＝結束往後＋下游連動、改開始日＝釘手填錨點、改結束日＝回算工期（§6.5），非反推移開始日。既有 interval 案首次進戰情室自動矯正重排。
  - **列間插入＝中途追加（範圍蔓延🌫）**：每列間 hover「＋插入任務」→ 成因窗（enum 六項必填）→ `scopeGrowthCount++`＋ecnEvents ＋走共用 `_s2InsertRow`（`_ecnInsertTask`/`_ecnCauseModal`）。
  - **表頭＝極淡琥珀底＋深字＋2px 深色底線**（原深底白字搶戲，Gemini 檢驗）；雙層表頭（前置任務跨欄＋三子欄）；斑馬紋 `--pearl`；hover 列 `--amber-l`＋左緣 3px inset；**關鍵路徑列＝左緣 4px 紅粗框＋極淡底**（原 terracotta 底與琥珀主題混淆）；數字 input **隱藏 spin 箭頭**；`table-layout:fixed`＋長字 ellipsis＋title。
  - **「操作」欄＝每列統一「⚙ 編輯 ▾」鈕** → 動作選單：**已完工列才有「↩ 打回重測」**、其餘列「✕ 刪除任務」；成因窗由選單觸發（入口看得見，不藏 hover）。**選單只留動作、不塞教學文字**（教學在 HintBox）；**刪除必經二次確認彈窗**「此舉將連帶影響 X 個後置任務的排程，確定刪除？」（X＝依賴此任務的後置數，不可逆操作防護網）。
  - **PM 常駐列**：琥珀底、「🔒 常駐·不可降級」、無操作鈕、鎖死儲存格 `cursor:not-allowed`（視覺封死編輯意圖）。**投入%＝該級預設（S15/M20/L40）可上調、不可低於預設**——高複雜案 PM 重負載（如 40%）要能如實反映，「不可降級」指不得低於預設與不得刪除。**實作：PM 列投入%＝可上調下拉**（`_ecnPmEffortOptions`＝該級預設 floor＋六檔中 >floor 者，可上調、不可低於 S15/M20/L40；反映高複雜案 40%+ 重負載，Gemini 檢驗①：不可寫死）；其餘儲存格 `cursor:not-allowed` 鎖死。名冊卡＝複用 `_s2DeptPanelHtml`（圖一「新增/編輯部門」鈕→`_s2OpenDeptModal` 彈窗，`_s2ApplyDepts` 加 ECN 分支寫回 proj.depts+存檔）。
- **異常變更互動（定版）**：
  - 中途追加＝**列間 hover 浮出「＋ 插入任務（記為中途追加）」橫線圓鈕** → 成因窗 → `scopeGrowthCount++` → 案內自動重排。
  - 打回重測＝已完工列「⚙ 編輯 → ↩ 打回重測」→ 成因窗 → 生成紅底重做列。**重做歸屬不靠前置判定、不用手動選**：按在哪列＝哪列的重做（`loopFromId` 自動綁定）；重做列前置預設接原任務（純排程用、可改）——`loopFromId` 與 `predecessor` 是獨立欄位。
  - **成因窗**（兩入口共用）：標題「記錄異常成因（必填，不可跳過）」＋成因 enum 下拉（§19.5 六項）＋備註一行（選填）＋「確認記錄」主鈕在最右＋fade-in 進入動畫；成因記錯→點該列「狀態/成因」欄虛線紅框小標籤（如「供應商 ▾」）重選。
- **定案：進度不進大表逐任務並列**——總體健康看左 HUD、大表守「排程與資源操作」職責分離。
- **施工註記（含 Gemini 終輪 6 條檢驗）**：① `.s2-tbl` 必帶 `table-layout:fixed`＋任務名 `ellipsis`（inline input 長字串防撐破）；② **「⚙ 編輯」動作選單禁純 CSS 絕對定位**——大表外層必有 `overflow-x:auto`，絕對定位選單會被喀嚓裁切，比照 Popover 用定位引擎掛 `body` 層級；③ 所有 input/select 定義 `:focus`（`outline: 2px solid var(--amber); outline-offset:-1px`）；④ 裸圖示按鈕一律 `padding: 6px 10px` 擴大點擊區（Fitts's Law）；⑤ select `appearance:none`＋統一自訂 SVG 箭頭（跨平台一致）；⑥ 控制項統一 `line-height:1.5`＋容器 `align-items:center`（垂直置中）。另：關鍵路徑列 hover 時左緣紅框優先於 hover 琥珀框（specificity）；彩色標籤字色一律同色系深字（WCAG AA）；成因彈窗 fade-in。

**C. 投入比例 HintBox 文案＋六檔判準表（貼大表上方；淺色收合式／走 :root）**
> ✅ reg A（2026-07-02）全站 HintBox 統一淡沙底 header＋白底 body＋深字＋收合（§6.5b）。**2026-07-02 改定：投入%＝六檔行為錨點下拉（§19.4），此 HintBox 兼當「評判標準」對照表。**

摘要四條：
1. **比例 ＝ 這項吃掉「負責人」一天多少**，不是佔整個設變案的比重。不用填數字、**選最貼的行為檔**即可。
2. **換算工時點數 ＝ 比例% × 每日工時 × 工期天數**。例：開模 20 天 ×6h×10% ＝12 點；改圖 4 天 ×6h×75% ＝18 點（點數＝真正吃掉的人力，非掛牌天數）。
3. **不用讓整張表加起來 100%**——每項各自獨立、不是分餅，加總 300%、500% 都正常。
4. **看爆不爆是「同一個人、同一天」**：他當天在跑的幾項比例相加 >100% ＝ 那個人被排爆（紅旗）。一個人本來就能同時扛多項、跨多案，系統負責疊起來示警。

六檔判準表（下拉評判標準；0＝掛名未出力舉證，§19.5）：

| % | 名稱 | 判準 |
|---|---|---|
| 0% | 掛名未出力（舉證） | 掛他名下但實際沒做，別人補上／擺爛 |
| 10% | 點一下就走 | 簽核／回信／一通電話，幾分鐘 |
| 25% | 零星盯著跑 | 發出去等回來，一天回點幾趟 |
| 50% | 半天在上面 | 開會／跨部門喬，佔半個工作天 |
| 75% | 大半天動手 | 改圖／彙整文件／現場試裝，這天主要在做它 |
| 100% | 這天只有這件 | 獨佔（趕樣機／談判當天） |

**D. 尚待 Stage 2 細做時定（洞7）**：迴圈🔄「前置未完成硬鎖 Disabled vs 只催辦/尊重手填」、自動順推遇手動釘錨誰贏、逃生口（手動解鎖）。屬 Stage 2 互動細節，不擋 Phase 1 開案/大表/BOM 地基。

**E. Mockup／落地狀態（2026-07-03 更新）**
- ⓪ **選型引導頁 ✅ 已落地**（§19.10 A.0，`4c4bb51`＋`ad8a73f` 迭代：Modal XL＋subgrid 對齊＋hover 琥珀）① **開案畫面 ✅ 已落地**（§19.10 A.1 三列式定版，`ad8a73f`＋修正批：教育卡綠框/上一步鏈/資料不保留警告/ECN 直落地）② **戰情室 dashboard Tab A ✅ 全落地並上線 `6aae2e0`**（2026-07-02，§19.10 B/B.1/B.2 定版·多輪線上驗 Pass：分流殼＋HUD 四卡＋共用大表全欄操作＋計畫/實際分離＋狀態衍生＋異常閉環＋⚙編輯彈窗）③ BOM·ROI 差異四區（§19.6，收進 dashboard 頁籤 B，**下一批施工前先設計**）。施工準＝§19.1b/19.4/19.5/19.6/19.9。
...
**Phase 1 施工**：✅ sidebar「設變案／ECN」群組（`71c398c`）→ ✅ 統一入口＋選型引導頁（`4c4bb51`，§19.10 A.0）→ ✅ **ECN 開案畫面＋範本引擎（`ad8a73f`，§19.10 A/A.1 定版）**→ ✅ 戰情室細節定案＋Mockup v2 定版（§19.10 B.1/B.2，2026-07-02）→ ✅ **戰情室 dashboard Tab A 全落地並上線 `6aae2e0`（2026-07-02，多輪線上驗 Pass）**：`renderProject` 依 `ecnType` 分流 `renderEcnDashboard`＋滿版佈局＋HUD 四卡（工時·異常統計·雙軌進度條·部門卡；`baselineHours`/`ecnEvents` 補 `_stage2Commit`）＋三頁籤；大表**複用 `.s2-tbl`/`_s2PredCells`/`_s2*` handler（hijack `_tplPreview`＋`_s2EcnPersist`）**——部門(雙向連動擔當)/擔當/前置三窄格/工期/**投入%六檔行為錨點**/**計畫日期(可編輯)**/**狀態(實際優先衍生·唯讀)**/⚙編輯彈窗（實際開/完工日·擱置+原因·打回重測·刪除）；列間＋插入=中途追加+成因窗；**ECN 一律 forward（`_ecnForwardVariants` 清 endDate）**；範本 effortRatio 歸六檔+生管；名冊複用 `_s2DeptPanelHtml`/`_s2OpenDeptModal`。**過渡已解除（開案即跳戰情室）。**
**下一件**：**大表投入% 批量修改**（Paul 需求，接著做）→ **全專案總覽「PM 跨案負荷」區塊**（§19.4 Phase 1 交付 b：PM 跨所有 active ECN 案常駐列日疊加→爆表紅燈；戰情室內不放導流連結、總覽的事在總覽看——Paul 定版）→ **BOM·ROI（Tab B，施工前先設計**：匯入舊/新→差異比對四區→切換方式/呆滯→目標成本→ROI→存資料不存檔 exceljs 重生）→ **結案流程/epoch 凍結/翻案重啟**（頁籤 C 事件時間軸已有殼）。
**獨立施工項**：✅ §6.5b HintBox 全站改淺色收合（reg A，`3778393`）；✅ Design Tokens 入 UI 規範 §6＋AGENT_GATE 規則 7（`07a638d`）。
**後續（另議，不擋 Phase 1）**：跨案 per-person 負荷圖（Phase 2，總覽側）；**受影響機種/多子案**（Phase 2 與 variant 架構一起：建議進 dashboard BOM·ROI 頁籤做「受影響機種」純資料列表＋每機種 BOM 分頁，§19.6 一案多 BOM）；**大表投入比例% 批量修改**（Paul 需求，dashboard 大表做完接著做）；BOM·ROI 泛化到 NPI；BOM 認定卡量產解鎖（Phase 2）；「設變背景與原因」維持選填（Paul 拍板，不加必填）。
- **【待做·Paul 2026-07-02】專案範本管理頁分 NPI／ECN**：把系統既有的兩套範本（NPI＝`PRODUCT_DEV_TEMPLATE`／ECN＝`ECN_TEMPLATE`）搬進設定頁範本管理（§8d.16 範本系統框架），**Admin 可直接在頁面編修範本內容**（階段/任務/工期/前置/投入%/角色/sizes），取代「改 Excel 給 Claude 更新 `ecn-template.js`」的手動流程。ECN 版要能編 S/M/L 條件開關（sizes）＋PM 常駐 sizeMeta。走 §8d.16.2 schema 集合化＋§8d.16.6 持久化四步。
