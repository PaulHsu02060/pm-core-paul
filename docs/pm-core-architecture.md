...
| token | 用途 | 套用欄 |
|---|---|---|
| `--col-num-w` | 短固定欄 | 序/#、狀態、進度%、餘裕、截止、工期、燈號、週次、單格日期 |
...
| 2 | 套 `.cal-row`（公休日，3 欄最單純）驗規範 | app.js render + css | 低 | 任一機 |
| 3 | 套 5 個現有 `<table>`：週報/任務史/PDCA 月報/Excel 匯入預覽/WBS 匯入預覽 | 各 render | 低-中 | 任一機 |
| 4 | 套 stage2 `.s2-tbl` | render | 中 | 任一機 |
| 5 | 待辦 `.task-grid` div-grid → `<table>`（互動移植：列點開/列間➕/hover/done-undated 摺疊） | app.js 大改 | 高 | 家裡桌機 |
