...
**第二刀（待辦）**：清資料層孤兒——`ensurePdcaData`/`ensurePdcaGroupsRoot`/`ensureTaskPdcaGroup`/`ensureAllPdcaData` 及呼叫、`STORE.pdcaGroups` load/save/sync、`task.pdcaGroup` 初始化。**保留 `proj.pdcaData.targetDate`（KPI 用）**；migration 不動。
