...
| 表 | 狀態 |
|---|---|
| WBS 匯入預覽 | 已套 .data-table，待轉 auto（拔 colgroup 固定寬） |
| Excel 匯入預覽 | 已套 .data-table，待轉 auto（拔 colgroup + compact） |
| 任務史 | 已套 .data-table，待轉 auto（拔 colgroup + compact） |
| 公休日（cal-table） | 已套 .data-table，待轉 auto（拔 colgroup） |
| stage2 s2-tbl | 已套 .data-table，待轉 auto + 表單欄位修正（input 吃滿、需交付欄置中、操作欄修正） |
| 待辦 task-grid | 未遷，div-grid → table（最高風險、互動移植，獨立 session 逐行核） |

轉 auto 統一處置：CSS 主體 table-layout 改 auto、拔 col 寬綁定三條、拔 compact、各表 render 拔 colgroup 固定寬。
