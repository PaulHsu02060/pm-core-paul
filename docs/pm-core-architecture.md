...
> ⚠ 後端 N 天保留上限、Sheet cell vs Drive 檔的存法，待後端施工時量實際 blob 大小定案，回填本節。
...
## 第十九部分：ECN 設變案管理模組（**設計草稿**，2026-07-01，待 RD/老闆拍板）

> ⚠ **本節為草稿**：branch 決策已先填「建議值」並標 `[待拍板]`，會議拍定後就地鎖死、移除草稿標記。
...
**🆕 新建（ECN 專屬層）**
- S/M/L 三範本、投入比例欄、負荷雙指標、迴圈/重啟紅旗、結案/版本快照狀態機、BOM 差額面板、per-person 跨案負荷圖（Phase 2）。

### 19.1 待拍板決策（會議鎖定，先填建議值）

| # | 決策 | 建議值 | 影響 |
|---|---|---|---|
| 核心 | 引入「投入比例%」欄？ | **同意** `[待拍板]` | 否決→比例欄與 Phase 2 負荷圖降級為後續；Phase 1 其餘照出 |
| (a) | 負荷拆兩指標並列 | **同意** `[待拍板]` | 畫面並列「人横切/案縱切」，不揉成一個 |
| (b)① | 誰填比例 | **範本帶預設→PM 可調（主）＋擔當可改自己（輔）** `[待拍板]` | 欄位權限 |
| (b)② | 範本預設比例 | **同意**（多一份範本維護工）`[待拍板]` | RD 交付物 |
| (b)③ | 既有任務 migration 比例 | **=100%**（保留現行 deptLoad 行為）`[待拍板]` | 不動現有專案負荷 |
| (c) | 部門欄 | **UI 不顯示、底層結構化存「人+部門」** `[待拍板]` | 撐 Phase 2 跨案負荷圖 |
| (d) | BOM 認定卡量產解鎖 | **方案一：Phase 1 純工具、卡鎖留 Phase 2** `[待拍板]` | BOM 不纏排程鎖 |
| 入口 | 進入點 | **既有「新增專案→選『設變案』範本」** `[待拍板]` | 複用路線B |
| RD | S/M/L 三範本內容 | **負責人＋交付日待敲** `[待拍板]` | Phase 1 能否蓋出 |

### 19.2 資料模型（新增欄位）

- **專案層**：`ecnType:true`、`size:'S'|'M'|'L'`、`ecnNo`（結案必填，如 ECN-2026-0042）、`effectiveDate`（簽核生效日）、`status:'active'|'closed'|'reopened'`、`loopCount`、`reopenCount`、`epochs:[{v, frozenAt, snapshotHours}]`（版本快照，洞4）。
- **任務層**：`effortRatio`（投入比例%，預設由範本帶；既有任務 migration=100%）、`isLoopTask:true`＋`loopFromId`（迴圈派生來源；平表用連結表達「巢狀」，不改資料結構為樹）。
- **BOM 層**（掛專案）：`bomRows:[{aPartNo,bPartNo,aPrice,aQty,bPrice,bQty,approveStatus:'pending'|'approved'}]`。

### 19.3 開案流程（複用路線B + Stage 2）

- **Step 1（3 秒開案）**：填機種名 + 選 S/M/L + （可選）開始/結束日 → 套 `_effScheduleDir` 三劍客試算，依 S/M/L 預設天數畫「評估→備料→驗證→結案」大框。
- **Step 2（區間預覽）**：既有甘特三色 + 時程不足警示。User 硬壓上市日 < 物理工期 → 觸發既有 `時程不足·超出 N 工作天` + 導 `_ovf*` 智慧面板。
- **Step 3（Stage 2 WBS 大表）**：自動帶 S/M/L 任務；欄位＝序號/任務名/擔當/前置/工期/**投入比例%**/預計日期(自動·鎖頭可切手動)/**迴圈🔄**/需交付。前置未完成→下游催辦（硬鎖互動見 §19.8 洞7，Stage 2 細部時定）。

### 19.4 投入比例 + 負荷雙指標（公式定案建議）

- **單任務工時** ＝ `effortRatio% × dailyHours × durationDays`（dailyHours 取 `DATA.settings.dailyHours`，現 6）。比例**可加總 > 100%**＝抓爆訊號。
- **指標 1（人的横切，「這人現在爆不爆」）** ＝ 某人 **across 所有專案** 當前 epoch 任務工時 ÷（dailyHours × 工作天）→ %。
- **指標 2（案的縱切，「這案燒多少」）** ＝ 單一案 **across 所有版本(v1+v2…)** 累計工時。
- **兩指標並列、不揉成一個**（(a)）。改既有 `Portfolio.deptLoad` 綠塊：由「工期×日工時(全投)」改「工期×日工時×**比例**」；既有任務比例=100% → 現行行為不變。

### 19.5 迴圈/重啟 + 結案/版本快照（洞4 模型）

- **異常迴圈🔄**：點擊 → 在下方派生重做任務（`isLoopTask`，`effortRatio` 帶入）→ `loopCount++` → 重跑排程順推 → 紅旗顯示迴圈次數。**迴圈不乘公式**（洞1），純加「實際派生任務工時」＋次數當風險旗標。⚠ 自動順推遇手動釘錨點不覆蓋（既有引擎①），衝突由 `_ovf` 面板處理。
- **結案**：填 `ecnNo` + `生效日`（強制必填）→ `status='closed'` → 當前 epoch 工時/工期凍結成 `snapshotHours` 快照。
- **重啟案外案🔄**：`status='reopened'` → 開新 epoch(v2)、v1 唯讀凍結；`reopenCount++`。**負荷：指標1 只算當前 epoch、指標2 加總所有 epoch**；紅旗（迴圈+重啟次數）跨版本累計＝血淚證據。

### 19.6 BOM 成本差額面板（Phase 1＝純工具）

- 快貼板 `[Ctrl+V]` 貼 ECN 差異有料號 → 解析成 rows（**新做**：既有貼上只到文字/截圖）；RD/採購補 B 單價用量。
- **整機差額** ＝ `Σ(B單價×B用量) − Σ(A單價×A用量)`，結果掛全專案總覽 KPI 卡（Cost Down/Up）。
- 匯出報價單（複用 exceljs）。
- `approveStatus` Phase 1 **僅顯示**；「認定未核可→卡量產解鎖」邏輯耦合留 Phase 2（(d) 方案一）。
- ⚠ 限制：兩欄式 row 表達不了 N:M 料號替代（1→2 件），輕量化取捨，Phase 1 接受。
...
### 19.9 S/M/L 範本內容（**骨架，待 RD 填**）

> 每套一張表；S/M/L 分級標準（看工期？看改不改模？）由 RD 定。比例＝該任務佔該人一天的投入%。

| 任務名稱 | 工期(天) | 預設比例% | 前置 | 需交付? | 階段 |
|---|---|---|---|---|---|
| _（例：改模）_ | _14_ | _80_ | — | — | 評估 |
| _（例：等部品認定）_ | _7_ | _10_ | _1_ | 認定書 | 備料 |
| … RD 填 … | | | | | |

（S 碼 / M 碼 / L 碼 各一張，RD 照此骨架複製填三套。）
