...
22. **問題3 步3：拔 J override 群本體 + synced/locked UI 橫掃**（接步1 拔 Sheet-API 同步、步2 拔核心 override 分支）。碰核心/UI 多，獨立 session、跑測試 + 線上驗。⚠ **動工前先盤資料**：
    - 先確認 DATA 有無殘留 J synced 任務（`proj.syncSource==='jSheet'` / `t.synced` / `t.locked`）；有殘留先決定遷移、沒有才安心拔。
    - override 群本體（app.js 1715-1767）：`J_OVERRIDE_FIELDS`/`isJTask`/`getJOverride`/`setJOverride`/`clearJOverride`/`getAllJOverrides`（步2 已拔核心依賴、剩 UI 呼叫，橫掃 UI 後一起拔）。
    - synced/locked/🔗 標記 15+ 處橫掃：cleanOldDoneTasks 豁免、排序加分、專案篩選、報表/PDCA 標籤、locked-modal(4711)、saveJSchedule/resetJOverride/resetAllJOverrides。
    - locked-modal(4711) 處置決策：唯讀／移除（步2 後其時程編輯行為已失效）。
    - 過時註解清理：mk doc（test ~444-447）、applySchedule（test 327 / app 1396-1397）的 override 字樣。
...
25. **cloudSyncToken 安全**：`settings.cloudSyncToken` 仍隨 doGet 公開 blob 外洩——剝離上傳 payload + 清雲端殘留 + 輪替 token。
...
28. **`.sync-status.error`/`.sync-status-time` 4 條 CSS 子規則順手清**：問題3 步1 留的孤兒（活元件 .sync-status 家族的 error/time 子規則，現孤兒、無害非急）。
