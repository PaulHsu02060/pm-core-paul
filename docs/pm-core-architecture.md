...
- **大表＝複用 Stage 2 真實結構（`_s2ListHtml`／`.s2-tbl`／`_s2PredCells`），琥珀化＋欄位調整**：
  - 欄序：序(38)/任務名(228，inline input＋關鍵路徑 tag)/部門(唯讀 role)/擔當(下拉名冊)/**前置三窄格(序號 select 64＋銜接方式 select 126＋緩衝 number 48——Stage 2 現成 inline 模式，Popover 方案廢)**/工期(number)/**投入%(number，佔原「需交付」欄位)**/日期起訖/狀態·成因(126)/**操作(96)**。
  ...
  - **PM 常駐列**：琥珀底、「🔒 常駐·不可降級」、無操作鈕、鎖死儲存格 `cursor:not-allowed`（視覺封死編輯意圖）。**投入%＝該級預設（S15/M20/L40）可上調、不可低於預設**——高複雜案 PM 重負載（如 40%）要能如實反映，「不可降級」指不得低於預設與不得刪除。
- **異常變更互動（定版）**：
