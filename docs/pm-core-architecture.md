...
- 三級燈號（分界已定案）：
  ...
  - 餘裕 0~4（≥0 且 <5）→ 黃燈「時間偏緊，任何延誤都可能超期」。
  - 餘裕 < 0 → 紅燈「時間不足，照此工期會超出結束日 N 個工作天」→ 觸發溢出三層（4.8.7.5）。
...
- 第一段：系統列關鍵路徑上長工期任務（只列關鍵路徑——壓非關鍵路徑對總工期無效），使用者改工期 → 按「儲存重算」。
  - 達標 → 完成。
  - 不達標 → 使用者手動按「展開全部任務」（不自動展開，67 筆太長）。
- 第二段（展開全部任務）：非一次全攤，比照層三 / 第二階段那樣切階段、一階段一階段改。此段改完即建立完成，不再回最後階段看一次（避免看兩次相同內容）。

儲存閘門（Q3，已定案）：
- 第一段改動不自動存，靠「儲存重算」。
- 進「展開全部任務」前，系統檢查有無未儲存改動：
  - 有未存 → 跳提醒「剛剛調整未儲存，展開全部後不會保留、需重填，確定？」（取消回去存／不保留直接展開）。
  - 無未存 → 直接展開。
...
- 塊3b：溢出層三（兩段式 + 儲存閘門，4.8.7.5 後半）。有狀態流程，獨立做。
...
7. **反推引擎（§4.8）**：✅ 已定案規格（§4.8 排程三模式 + backward pass + 區間餘裕 + 溢出三層 + 階段三升級，2026-06-23）。施工分塊：塊1 schema（targetEndDate/scheduleMode，✅ 49a89fe）→ 塊2 backward pass 核心+Node測試（✅ 0cc65fa，133綠）→ 塊3 區間/餘裕/溢出（🔄 接線中，規格見 §4.8.7）→ 塊4 懶人式UI。需家裡桌機 Node。
