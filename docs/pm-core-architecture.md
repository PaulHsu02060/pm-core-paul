...
- **任務層**：`effortRatio`（投入比例%，預設由範本帶；非範本手動新增或既有任務 migration=100%）、`isPmCoord:true`（PM 常駐協調列標記，Model Y，§19.4）、`taskAttr:'baseline'|'conditional'|'fog'`（基礎常駐/條件常駐/迷霧衍生，§19.9）、`isLoopTask:true`＋`loopFromId`（迴圈派生來源；平表用連結表達「巢狀」，不改資料結構為樹）、`causeTag`（蔓延/迴圈事件成因 enum，Hybrid 強制確認，§19.5）。（`expectedProgress`/`actualProgress`/`variance` 屬 runtime 動態算，不入範本 schema。）
- **BOM 層**（掛專案）：面板 `bomBaseCurrency`／`bomQuoteCurrency`／`bomRate`（雙幣別＋匯率）、`baselineFromBomId`（撈舊版 fork 來源，可空）、`annualVolume`（年產量，手填）、`oneTimeCost:{mold, cert, deadStock}`（一次性設變費用拆項，§19.6）；`bomRows:[{partNoA, replacePartNoB, oldPrice, oldQty, newPrice, newQty, approveStatus:'pending'|'approved'}]`（萬用 row，見 §19.6）。
...
### 19.6 BOM 成本差額 + ROI 雙軌工具（Phase 1＝純工具，內頁右側可收合抽屜）

**Baseline 起點（撈舊版）**
- 起點可**撈歷史舊版 BOM fork 當設變 baseline**（熱泵故事：PM 手動撈「無輪子版」BOM→加料）→ 系統若存歷代 BOM，一鍵 fork（`baselineFromBomId`）省半天手工；無歷史則空表手建。

**萬用 row schema**（一列通吃 同料改量／換料 A→B／增料／刪料，§19.2 `bomRows`）：`partNoA`／`replacePartNoB`(可空)／`oldPrice`／`oldQty`／`newPrice`／`newQty`／`approveStatus`。
- 同料改量：B 空、單價同、用量新舊不同；換料：B 填；增料：舊用量 0；刪料：新用量 0。
- ⚠ 1→2 料的 N:M 替代要拆兩列（輕量化取捨，Phase 1 接受）。
- **單行價差 ＝ `newPrice×newQty − oldPrice×oldQty`**；「本次設變單顆成本總差額 ＝ Σ 單行價差」（換基準幣後）。

**ROI 雙軌（決策支持，非只算單顆）**
- 開案時分流**效益型／被迫型**（`roiType`，可由 `changeReason` 推）：
- **效益型（Cost Down）**：`年效益 ＝ 單顆總價差 × 年產量`；`回本期(月) ＝ 一次性設變費用 ÷ 年效益 × 12`；並顯示 舊成本／新成本／目標售價／客戶可接受漲幅 → 毛利影響（OEM 客戶主導時決策＝漲價／自行吸收／用料補償）。
- **被迫型（停產/法規/設計錯誤）**：回本期無意義（分母負或零）→ 改顯示「**合規/變更總代價**」，驅動壓最小損失（充電樁法規案＝避免停線的代價）。
- **一次性設變費用拆項**（`oneTimeCost`）：模具費＋認證費＋**呆滯料損失（＝舊料庫存量×舊料單價，接 §19.5 生效日卡控）**。
- **年產量**（`annualVolume`）＝手填欄（PMC/業務數，註明來源，PM 未必知）。

**幣別／匯入／匯出**
- **雙幣別＋匯率彈性換算**：頂端「報價來源幣別」下拉（USD/NTD/RMB/JPY）＋「換算基準幣別」下拉（NTD/USD/RMB）＋動態匯率填欄（`1 [來源] = [值] [基準]`，幣別跟下拉連動、數值留白由 User 填當日匯率）；單行與總差額一律自動套匯率換基準幣。
- **Excel 智慧快貼/匯入**：貼 Excel 矩陣 → 自動對齊欄位解析成 rows（新做；既有貼上只到文字/截圖）；配 **`🗑清除重貼`** 按鈕（貼錯格式一鍵清空重來，防卡死）。庫存快照（生效日卡控用）走同一貼上軟橋。
- **匯出買料清單**（複用 exceljs／`exportProjectWbs` 路徑）；總差額掛全專案總覽 KPI（Cost Down/Up）。
- **多交付軌**：一設變可分「新機 BOM／外場重工包 BOM」兩軌（`deliveryTracks`），各自 row set/SOP/結案（充電樁：產線新機＋外場 FAE 重工包）。
...
**B. 專案內頁（畫面二，三區塊：左 ~25% / 中 ~75% / 右 可收合抽屜）**
- **左側（本案統計＋版本；正名·拒絕跨案人均放此）**：
  - 兩統計：**本案當前在途工時**（當前版本、`status!=='done'` 才計）＋**本案歷史累計工時**（跨版本 v1+v2… 不歸零）。（真正的「某人跨案爆不爆」＝人橫切，放 Phase 2 全專案總覽跨案負荷圖，不放此。）
  - 版本切換 v1/v2；狀態標籤（進行中/已結案/**已封存·唯讀**）；紅旗（重啟/迴圈 `loopCount`>0 亮紅）；部門/負責人名冊。
  - HintBox：v1/v2 旁 ℹ「切換版本可檢視歷史唯讀快照；重啟新版本會繼承前版進度並凍結舊版」；紅旗 ℹ「本案因設變重開次數，工時跨版本累計、留血淚證據」。
- **中間（甘特＋大表；整套搬既有專案範本 Stage 2 Component）**：
  - 上段 **甘特預覽**（區間 ＋ 時程不足三色燈，複用 `_ovf`）。
  - 下段 **大表欄位**：序號／任務名／**預計開始**／**預計結束**／工期／**投入比例%**／擔當（顯示名字·底層綁人+部門）／實際進度狀態／需交付／迴圈🔄。
  - **編輯權限鎖**：工期／投入比例%／擔當 三欄 hover 出 ✏️、點擊直接 Input，改完觸發引擎自動順推重算日期。（比例權限：PM 可調全部、擔當僅改自己那格。）
  - **狀態欄強制唯讀**：只顯示 未開始/進行中/已完工，連動底層 `status`，**完工自動退出在途工時**（不給手動改）。
  - 大表上方掛 **投入比例 HintBox**（見 C）。
- **右側（BOM 差額工具，可收合抽屜，Phase 1 純工具不卡鎖）**：欄位/幣別/流程見 §19.6。

**C. 投入比例 HintBox 文案（貼大表上方，§6.5b 公版：深色 Header／ol 數字序號／body 12px／走 :root）**
...
**E. 三方碰撞增修（2026-07-01）——對本節 Mockup 規格的修訂，以 §19.1b／§19.4／§19.5／§19.6／§19.9 為準**
- **開案分級**：選範本下拉的「S/M/L」＝同瘦骨架的條件任務預設開關＋工期檔位（§19.9），非三張獨立範本；開案為初始猜測，評估後 Gatekeeper 可升/降級。範本任務不再是「猜滿的胖清單」而是 6 階段瘦骨架＋條件/迷霧池。
- **內頁左側**：除本案在途/累計工時，加 **PM 跨 active ECN 案負荷（爆表紅燈，§19.4）**＋蔓延🌫/迴圈🔄/重啟 三旗與成因彙總（§19.5）。
- **內頁大表**：欄位加 **預期進度／實際進度／雙軌變異（落後 vs 蔓延）**；Gatekeeper 檢核與成因標記（Hybrid 強制確認）內嵌於對應任務列；`taskAttr` 標 baseline/conditional/fog（§19.5/§19.9）。
- **右側 BOM 抽屜**：升級為 BOM＋ROI 雙軌（baseline fork／效益·被迫兩態／回本期／呆滯連動生效日／多交付軌，§19.6）。
- **開案「呈現層」（方向 C）已 Mockup v4 定版（2026-07-01），權威規格見 §19.10 A.1／A.2**：骨用完整 `_flowStep1`、S/M/L 具現為分級按鈕(動態即時反饋)、名冊/主案收左側、日期＋Timeline 綁右側、拔底部擔當下拉、PM 協調條入甘特、HintBox 輕量收合。內頁(B)／BOM 抽屜(§19.6) Mockup 待續。
