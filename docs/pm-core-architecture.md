...
> 最後更新：2026-06-21（家裡桌機，基準 HEAD `ffc8e4f`）
...
**階段二：甘特填色（後做）**
同表右側時間格子填色畫甘特條（MS Project 風格）。ExcelJS cell.fill solid + 凍結窗格 frozen。
待定（TBD，階段二開工前定）：
- C1 時間粒度：日/週/月（92 筆跨年用日＝300+欄不可行，傾向週或可選）
- C2 plan/actual 擺法：甲一任務兩列（忠實、列數×2）/ 乙一任務一列兩色（省列、actual 蓋 plan）
假日欄底色讀 DATA.calendars 暖灰；顏色寫匯出模組 JS 常數（Excel 讀不到 :root 變數，合理例外、集中一處）。
