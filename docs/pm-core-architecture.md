...
- **逐日掃本週**（`D.monday()`～`sunday`），三 store 各自判該日是否有該會議、命中即累加 `H_meeting` 到 `chore`：
  - `recurringMeetings`（`enabled!==false`）：每天用現成 `eventOccursOnDate(m, dayIso)` 判（單一真實來源，與週曆/排程占位同函式；自動吃 frequency／startDate／endDate）。每週會議自然算 1 次、每天會議算 5 次。
...
**已知邊界**：① 會議無「人數」概念，占用區間制刻意不乘人數 ② 全體均攤在「全系統無任何專案部門」時無處可疊（罕見、跳過）③ `owner` 本刀只存不算，未做跨人負載 ④ 同 §18.10 副作用：橘塊只反映「已記錄並掛部門」者、必有漏算（UI 已明示）。
