...
**A.2 開案小幫手 HintBox（輕量收合式，context-specific 例外，異於 §6.5b 深色公版）**

- 定位＝開案頁 inline helper：**淡底 `#F1EFE8`＋sage 深字＋500 輕量＋預設收合**，只留一行「💡 開案小幫手 · 點擊了解分級與時程推算邏輯」，避免壓過必填欄。
- ⚠ §6.5b **深色 Header 公版仍用於區塊級說明框**（如內頁大表上方投入比例 HintBox）；開案 helper 因需退為輔助而輕量化，屬 context-specific 例外，不改 §6.5b。
...
> ⚠ **reg 決策 A（2026-07-01 定）：全站 HintBox 統一改「淺底＋深字＋預設收合」**，§6.5b 更新、含已上線頁一併換（獨立施工項，見 §19.11 後續清單）。以下四條文案不變。
...
**Phase 1 施工（按三張 Mockup）**：開案畫面（複用 `_flowStep1`＋ECN 三處＋名冊挪前＋分級按鈕＋PM 協調條＋排程建議行＋Fan-out）→ 建立即落地跳 dashboard → sidebar 新增「設變案」群組 → dashboard（HUD＋3 頁籤，大表前置走白話三欄、成因 Enum、迴圈列、PM 常駐列）→ BOM·ROI（匯入舊/新→差異比對四區→切換方式/呆滯→目標成本→ROI→存資料不存檔 exceljs 重生）。
**獨立施工項**：§6.5b HintBox 全站改淺色收合（reg A，含已上線頁 code＋memory `hintbox-writing-standard` 更新）。
