...
7. **反推引擎（§4.8）**：✅ 已定案規格（§4.8 排程三模式 + backward pass + 區間餘裕 + 溢出三層 + 階段三升級，2026-06-23）。施工分塊：塊1 schema（targetEndDate/scheduleMode）→ 塊2 backward pass 核心+Node測試 → 塊3 區間/餘裕/溢出 → 塊4 懶人式UI。需家裡桌機 Node。
