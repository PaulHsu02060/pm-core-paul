...
**第一優先：B 系列收尾（地基，低風險）**
1. B-2 專案頁接分段鈕（範圍=單專案）：`renderGantt` 傳 `Set([currentProjectId])` + targetId、`renderMonth` 傳 targetId。專案頁長出 Gantt/月曆視圖的前置。
2. B-3 `renderMonth` 月曆解耦：單專案範圍的 task/meeting 篩選。B-2 月曆視圖需要。

**第二優先：核心大功能（要先出設計文件）**
3. 專案 Task By Stage 三視圖（看板/Gantt/清單，參考舊版 WBS）：依 stage 分組，三種呈現切換（看板拖卡 / Gantt 時間軸 / 清單表格）。依賴 B-2 分段鈕框架落地。動工前須先產獨立設計文件（三視圖呈現規格、By Stage 分組邏輯、與現有 renderGantt 共用方式）。

**第三優先：UI 微調（純 CSS，動工前出 mockup）**
4. 總儀表板分段鈕放大 + 配色：現 `.tab-btn` 太小不顯眼，放大並配色讓可點性明確。
5. 週報專案資訊欄對齊：每個專案的資訊欄未對齊，調版面。

**第四優先：整理（低優先）**
6. 設定頁 v2 重規劃：選項雜亂，盤點分類；側欄預設收合。
