...
- **Step 2（開案表單）＝複用既有「套用範本創建」_s1 頁**（勿自創）：頂端常駐**型別 Banner**（綠=開發／琥珀=設變，寫「建立後無法轉為 X」，§19.10 A.1）；填名稱/顏色/主案子案日期 → **排程列直接複用 _s1／`.s1-dynhint`**（日期、正逆推、餘裕同資料同邏輯，僅配色貼合，`_effScheduleDir`），**開始日/上市日擇一必填**（範本＋設變適用；空白/Excel 不強制日期）；畫 6 階段骨架（立案評估→設計變更→部品認定→驗證測試→客戶決策→生效結案，§19.9）＋甘特預覽/時程不足。設變**只加** 原因分類、`ecnNo`(可空)。
- **Step 3（建立即落地）**：點「建立」→ 開發案落地一般專案 dashboard；設變案**自動落地並跳該案戰情室 dashboard**（§19.10 B），並歸入 sidebar「設變案 / ECN」群組。dashboard 大表欄位＝任務/擔當/工期/**前置（白話三欄·不出現 FS，複用 `_predRowHtml`）**/**投入比例%**/預計結束/狀態·成因；**拿掉需交付欄**。前置未完成→下游催辦（硬鎖互動見 §19.8 洞7）。ECN＝簡化實體，無看板/甘特/月曆 view 工具列。
...
- **視覺**：全頁**純白底**、卡片**俐落細線框**（NPI 卡 `--sage-100` 淺綠邊／ECN 卡 `--amber-border` 淺琥珀邊）、**無厚陰影**；群組色＝開發綠 `--sage`／設變琥珀 `--amber-accent`，對齊 sidebar 色點；分級色點 綠→琥珀→玫瑰（`--sage-500`／`--amber-accent`／`--rose`）＝輕→重。全色走 `:root`。
...
- **選子卡 → 進 §19.10 A/A.1 開案表單**：開發子卡走 _s1 三種既有帶任務方式（範本/Excel/空白）；設變子卡帶 §19.9 對應 S/M/L 骨架。**空白/Excel 不強制日期**（見 A.1 日期規則）。
...
- **型別 Banner（頂端常駐，2026-07-02 定版）**：選型頁（A.0）選完卡片進表單 → 麵包屑下方常駐一條型別 banner，**填表全程不退場**，依分類切換文案、色走群組色（開發綠 `--sage-50`／設變琥珀 `--amber-l`），**取代「按建立時再彈確認」**（banner 全程在，User 不會填完才發現選錯型別、不能改要重開）：
  - 開發（綠底）：「📁 您正在建立：產品開發案(NPI) — 採一般專案排程模式(含甘特／WBS／月曆檢視)。建立後無法轉為工程設變案。」
  - 設變（琥珀底）：「⚙ 您正在建立：工程設變案(ECN) · S 級 — 啟用設變專屬戰情室(健康 HUD＋進度變異＋BOM·ROI 三頁籤)。建立後無法轉為開發案。(S/M/L 分級開案後仍可調整)」
- **排程列＝直接複用 _s1／`.s1-dynhint`（不另做一套，鐵則：可共用模板資料一律拿出來共用）**：日期欄位、正逆推、餘裕計算的**資料來源與邏輯完全同源**，頂多配色貼合設變琥珀。助攻文案（欄位上方）「請先輸入已知資訊，系統將依據您填寫的日期自動推算排程。」；inline hint 文案「僅填開始日→系統依各階段工期順推預計完工日；僅填上市日→逆推最晚必須啟動的日期；兩填→精算餘裕、超死線亮紅」。**日期必填規則：開始日/上市日擇一必填或兩填（範本＋設變案適用，同套用範本規則）；空白/Excel 不強制日期**（無階段順推意義低、Excel 自帶日期）。
...
**E. Mockup 定版狀態（2026-07-01；2026-07-02 增選型頁）**
- **四張全定版**：⓪ **選型引導頁（Wizard，2026-07-02，§19.10 A.0）**——統一入口取代兩群尾＋鈕、兩欄型別卡、純白線框、全套文案定版 ① 開案畫面（方向 C，§19.10 A.1/A.2；2026-07-02 加型別 Banner＋排程列複用 _s1）② 設變專屬 dashboard（左 HUD＋右 3 頁籤，§19.10 B）③ BOM·ROI 差異四區（§19.6，收進 dashboard 頁籤 B）。往後 Phase 1 施工按此四張＋§19.1b/19.4/19.5/19.6/19.9。
...
**Phase 1 施工（按四張 Mockup）**：✅ sidebar 新增「設變案／ECN」群組（`ecnType` 分群，V3 區塊底帶群標，`71c398c`）→ **下一件 入口重建＋選型引導頁（§19.10 A.0）**：sidebar 頂端單一「＋ 建立新案」鈕、移除兩群尾＋鈕、兩群退為 View 篩選器、選型頁兩欄型別卡（開發 範本/Excel/空白｜設變 S/M/L）＋不可互轉警示＋防過勞條→ 開案表單（複用 `_flowStep1`＋頂端型別 Banner＋ECN 三處＋名冊挪前＋分級按鈕＋PM 協調條＋排程列複用 `.s1-dynhint`＋擇一必填＋Fan-out；`openEcnDialog` 現為占位 toast）→ 建立即落地跳 dashboard → dashboard（HUD＋3 頁籤，大表前置走白話三欄、成因 Enum、迴圈列、PM 常駐列）→ BOM·ROI（匯入舊/新→差異比對四區→切換方式/呆滯→目標成本→ROI→存資料不存檔 exceljs 重生）。
