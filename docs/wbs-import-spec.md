...
2. 前置（本階段）：predecessor 維持存 WBS 序號，跟現有引擎一致。「插入不亂」的前置 id 化重構列入未來核心升級，本次不做。
3. 重複匯入：清空舊 J 任務整批重灌（甲）。第一次不影響。
4. 專案資訊分頁：一起讀，自動填專案的性試/量試/量產結束日（資訊條來源）。
...
4. node 驗：93筆齊、前置 2FS+2/118,119 對得上序號、階段/類型/風險議題正確、專案資訊填進資訊條
5. 全對 → commit
