# PM-Core 系統架構設計文件（主文件）

> 本文件是 PM-Core 的**單一架構真實來源**。整合所有定案設計 + 已完成進度，只保留最新正確版本。
> 施工前必讀；定案內容不憑記憶改動；新需求若與本文件衝突，以討論更新本文件為先。
> 最後更新：2026-06-09（基準 HEAD `57de432`）

---

## 第零部分：專案最高原則

**整體系統的「邏輯乾淨、正確、單一真實來源、不重複」是最高原則。**

由來：前兩版舊系統（各自有 bug、無法合併）打掉重做，才有這第三版 PM-Core。所以「乾淨/不重複/可合併」不是潔癖，是專案誕生的理由。重蹈「兩份各自演化、各自有 bug、合不起來」的覆轍 ＝ 專案失敗。

衍生原則：
- **抽共用 vs 複製兩份 → 一律選共用**（重構打底）。
- **不重構，除非底層假設錯** —— 防的是對穩定 code 亂改結構；但當新需求即將長出重複時，先合併再加＝正解，不違反此鐵則。
- **計算層純資料**：core 函式只回傳資料，不碰 DOM、不碰 Storage；呈現分流不影響計算。
- **每個計算結果都帶推理依據**：warning、reason、startSource、anchorSource 都顯式。
- **真實使用情境優先**：假設只有 PM 一人維護資料，圍繞實際用法設計。

---

## 第一部分：四層架構

```
Core（純排程/計算邏輯，無 DOM、無 Storage）
  ↓
UI（渲染層，消費 core 算好的資料）
  ↓
DB（Storage / 雲端同步）
  ↓
Auth（檢視/編輯/登入）
```

分層紀律「從第一天就遵守」，即使 code 物理上還在單一檔案：core 函式回傳資料，呼叫端決定渲染或儲存。`computeSchedule` 直接呼叫 `renderGantt` + `Storage.save` 是要避免的反模式。物理拆檔之後是機械搬移，不是重新架構。

---

## 第二部分：核心架構決策 —— 雙視圖模型（2026-06-06 定案）

### 2.1 問題根源

系統裝著兩種本質不同的任務，硬用同一套欄位描述會打架：

- **時段制任務**：日常瑣事 + 會議。天生是「某天某時段做、花幾小時」，要塞日曆格子、可拆分。
- **工期制任務**：WBS 專案項目。天生是「佔幾個工作天、有前置依賴」，是甘特區間。

用工時描述 WBS → 算不出甘特依賴；用工期描述瑣事 → 拆時段無意義。**一個欄位扛不起兩種語意。**

### 2.2 解法：雙視圖

| 視圖 | 回答的問題 | 內容 | 計量 | 呈現 |
|---|---|---|---|---|
| **視圖一：行事曆/時間軸** | 我今天/這週要做哪些、怎麼排時間 | 會議 + 勾「排入行事曆」的任務 | 工時（H）+ 時段，可拆分 | 時間軸/日曆格子 |
| **視圖二：專案進度/待辦** | 所有專案項目進度、什麼快逾期 | 其餘所有任務 | 工期（工作天）+ 區間 | 甘特 + 逾期/待辦清單 |

兩視圖切換或並列；各用自己合理的計量，不互相打架。

### 2.3 分流規則

| 任務 | 分流 | 判斷依據 |
|---|---|---|
| 會議 | 視圖一 | 來自 meetings store，自動 |
| 勾了「排入行事曆」 | 視圖一 | 新增欄位，手動勾 |
| 其餘所有任務 | 視圖二 | 預設 |

**核心洞見：分流的分界不是「屬不屬於專案」**（因為瑣事也常來自專案的細項），**而是「我要不要親自排時間動手做」。**
- 同樣掛 J 系列：「我要追進度的」留視圖二；「我要動手做的細項」勾「排入行事曆」進視圖一。
- 新增欄位：**「排入行事曆」**（布林勾選，手動建任務時決定）。

### 2.4 分流只管呈現，不管計算

**部門負荷、KPI、報表是獨立計算層，吃全部任務，不受分流影響。** 一筆任務無論顯示在哪個視圖，都是「你做的事」，都要進計算。

---

## 第三部分：部門負荷計算（2026-06-06 定案）

**統一單位：H（小時）**，所有任務換算成 H 後相加到該擔當：

| 任務類型 | 負荷（H） |
|---|---|
| 時段制（排入行事曆） | 直接用工時 H |
| 工期制（WBS） | `durationDays × dailyHours`（每日工時，`DATA.settings.dailyHours`，預設 6） |
| 會議 | 會議時長 H |

換算邏輯已存在系統（匯入器的 `estHours = durationDays × dailyHours`），直接沿用。

**重點：**
- 不論有沒有勾「排入行事曆」，**全部算進負荷**。
- **工期 H 要按區間攤平到每個工作日**，不可整包壓在開始那天（15 天 = 90H，是攤在 15 天裡，不是今天壓 90H）。

---

## 第四部分：排程引擎（已完成，56/56 PASS）

### 4.1 計量定義

- **工期（durationDays）**：任務本身佔幾個工作天。引擎核心輸入，`end = addWorkdays(start, durationDays - 1)`。決定後續任務何時能接。獨立欄位，不可與前置 lag 共用。
- **前置 lag（+N）**：前項完成後「等」幾個工作天才開始本項（等材料、等簽核）。是任務「之間」的間隔，不是任何任務的長度。UI 預設隱藏，可點開填。
- 兩者本質不同：例「A 完成後等 3 天，B 自己做 5 天」→ lag=3、工期=5，各管一段。

### 4.2 前置依賴關係（FS/SS/FF/SF）

- FS（完成才能開始）：前項完成後本項才開始（最常用）。唯一需要 +1（SOD/EOD 模型）。
- SS（同時開始）/ FF（同時完成）/ SF（開始才能完成，少用）：+0。
- `parsePredecessors` 支援純編號與 FS/SS/FF/SF+lag 格式。

### 4.3 排程方案（丙 + 逃生口）

- **computeSchedule**：純計算，尊重手填日期（不覆蓋），只在空白時從前置推算開始日，無 start 無前置標「待排」，輸出建議串。
- **applySchedule(tasks, scope)**：scope = `'manual'|'one-level'|'full'`，甲/乙/丙 全是參數。
  - 甲：手動逐筆，逃生口
  - 乙：套一層（日常維運）
  - 丙：整鏈套用（初次匯入 92 筆最實用），預設推薦
- 寫回 `plannedStart/plannedEnd`，**絕不碰 `task.start`**（碰了會誤鎖成手填錨點，下游不跟動）。
- 只套 `blocked===false && suggestedStart!=null`，blocked/循環/手填錨點自動跳過留警示。

### 4.4 α 方案錨點分流

- J 任務讀 `override._localStart`，手動任務讀 `t.start`（修正「92 任務全錨定」bug）。
- 錨點跳過 scheduled 寫入（scheduled 是純機器層）。

### 4.5 getEffectiveSchedule 優先序

`override > actual > scheduled > planned`，並帶 `startSource`。
**用 `||` 不用 `??`**（override 可存空字串，nullish coalescing 會誤判）。

### 4.6 逾期判定（待施工，§10 第 2 項）

改口徑為 `(deadline || plannedEnd) < today 且 status !== 'done'`（擱置 hold 排除）。現況散落 4 處（:2245/2562/2963/4552），要統一改、不可漏。

---

## 第五部分：狀態衍生規則

四值：`pending`（未開始）/ `wip`（進行中）/ `done`（已完成）/ `hold`（擱置中）。

自動推導（實際日期優先於狀態欄）：
- 有實際完成日（actualEnd）→ 強制「已完成」
- 有實際開始（actualStart，無完成）→ 強制「進行中」
- 皆無 → 看狀態欄
- 逾期非狀態，即時推導（見 4.6）

UI：狀態欄反灰唯讀，`?` hover 說明規則。

---

## 第六部分：任務表單（M2，三塊已完成 + 待施工項）

### 6.1 共用架構

- 由 `App.buildTaskFormHtml(task, mode)`（app.js:3217）產生，new/edit 共用同一份。單一真實來源，禁止複製兩份。
- 儲存函式維持兩個（`saveNewTask`/`saveTask`），不合併；跨模式可能讀到不存在欄位處加 null 防呆。
- locked 版（synced 唯讀）不套此設計。
- 驗證訊息 house style：`U.toast('⚠️請填XXX', 'warning')` + 直接 `return`。

### 6.2 欄位與排序

| 順序 | 欄位 | 必填 | 元件 | 狀態 |
|---|---|---|---|---|
| 1 | 專案 | ✅ | select | ✅ 完成（改名、移到最上） |
| 2 | 任務名稱 | ✅ | text | ✅ |
| 3 | 說明 | — | textarea | ✅ |
| 4a | 擔當 | ✅ | text | ✅ |
| 4b | 類型 | ✅ | select（`?` 掛 data-tip） | ✅ 完成（`?` hover data-tip 即時說明） |
| 5a | 階段 | ✅ | text+datalist | ✅（改名 PLM階段→階段） |
| 5b | 子群組 | — | text+datalist | ✅ |
| 6a | 緊急程度 | — | select（`?` 掛 data-tip） | ✅ 完成（`?` hover data-tip 即時說明） |
| 6b | 狀態 | — | select（`?` 掛 data-tip） | ✅ 完成（`?` hover data-tip；反灰唯讀仍待做） |
| 7 | 前置任務 | — | 結構化下拉（見 6.4） | ✅ 完成（結構化 + UI 重設計，serializePredecessors 共用） |
| 8a | 預計開始 | — | 自動/手動雙態（見 6.8） | ✅ 完成（方案甲；放掉必填） |
| 8b | 工期 | — | number | ✅ 欄位 / ❌ 自動算未做 |
| 8c | 預計完成 | — | date（自動算） | ❌ 自動算未做 |
| 9 | Deadline | — | date | ❌ 新欄位未做 |
| 10 | 需拉高層 HL | — | checkbox+textarea | ✅ 完成 |
| 11 | 實際執行區 | — | 反向摺疊 | ✅ 完成（含交付物） |
| 新增 | 排入行事曆 | — | checkbox | ❌ 未做（雙視圖分流用） |

**已移除：** 分類（category，UI 拿掉、資料層保留、行事曆配色仍讀 category）、處理方式（method）。

**必填（5）：** 專案、任務名稱、擔當、類型、階段。已完成。（預計開始改自動/手動雙態後放掉必填，見 6.8。）

**欄位大小：** 統一 38px（前置列內 36px）。

### 6.3 類型/緊急/狀態說明（✅ data-tip 即時 hover）

- 任務：有工期、要排程的實際工作項目
- 里程碑：時間點標記（工期 0），如審查、交付節點
- 群組：純分類母項，不參與排程
- 里程碑/群組的 category 給空。
- **三欄的 `?` 圖示掛自製 `data-tip`（非原生 title），hover 150ms 即時顯示深綠卡片（C 方案）。** 文案（格式「標題|內文」）：類型「類型\|任務=要排程的工作；里程碑=時間點標記（工期0）；群組=純分類母項，不排程」、緊急「緊急程度\|系統自動推算，可手動覆蓋」、狀態「狀態\|依實際開始/完成日自動推導」。HL「需拉高層」的 `?` 也一併從原生 title 改 data-tip。

**欄位說明統一規範（C 方案，2026-06-09 定案）：**
- **單句說明** → 自製 `data-tip="標題|內文"`（`initTooltip` 即時 hover 150ms 深綠卡片；**禁用原生 `title`**）。
- **多句或需範例引導的複雜欄位**（如前置任務）→ 常駐 `.field-hint` 文字 + `.tip` 範例卡。
- **功能性 `title`**（本地調整標記、刪除鈕等非說明性）→ 不在此規範內，保留。

### 6.4 前置任務結構化（✅ 完成：結構化下拉 + UI 重設計）

取代舊自由文字 `1FF,2FS+2`（沒人會填）。

**結構化下拉（每條一張卡片，三欄垂直堆疊）：**
- 🔗 要接在這個任務之後：搜尋 input（datalist，模糊比對「編號 · 名稱」）+ ✕ 刪除鈕。
- 🔀 兩者的銜接方式：select，**整句白話**（「等它完成後，本任務才開始（最常用）」=FS／「跟它同一天開始」=SS／「跟它同一天完成」=FF／「它開始後，本任務才能完成」=SF）。
- ⏳ 中間留幾天緩衝：number 常駐（預設 0，不再用「+延遲」收合）+ 說明小字。
- 加列／刪列（多條前置）；候選只列本專案、有 wbs 編號的任務（手動無編號暫不能當前置）。
- 頂部總說明 + 底部 `.tip` 範例卡（常駐，取代舊 `?` hover）。

**序列化（單一真實來源）：**
- `serializePredecessors()`（DOM 列 → 字串）+ `parsePredecessors`（字串 → 陣列，反序列化）兩存檔點共用。
- 格式維持 `16FS / 16FS+2 / 16SS`，**引擎 parsePredecessors 完全相容、零資料改動**。

**commit：** `71c7e78`（結構化下拉）+ `ff3a3fd`（UI 重設計 + `?` 改常駐小字）。

### 6.5 預計完成自動算（未做）

- `預計完成 = addWorkdays(預計開始, 工期 - 1)`，跳國定假日+公司行事曆。
- 預計開始+工期填完自動帶出，可手動覆蓋。

### 6.6 Deadline（未做）

- 新增欄位，手填截止日。
- fallback：`deadline || plannedEnd`（沒填取預計完成）。
- 可從 Excel 匯入（模板要多一欄、匯入器要補讀）。

### 6.7 HL + 實際執行（已完成）

- HL：單一布林 `riskHL` 勾選 → 展開 `riskIssue` 文字欄。
- 實際執行：CSS 反向摺疊（new 收起、edit 展開，DOM 永遠在避免裸讀炸），內含實際開始/完成 + 交付物（`deliverable` 文字 + `deliverableLink` 連結）。原生檔案上傳未做（無檔案後端，未來接雲端 storage）。

### 6.8 預計開始：自動/手動雙態錨點（✅ 方案甲，已完成）

預計開始改「自動／手動」雙態，對接排程引擎的錨點機制（`task.start` 有值＝錨點、空＝非錨點由前置推算）：

- **預設「自動」**：`task.start=''`，引擎依前置推算開始日（自動態 UI 灰底唯讀顯示，接引擎後填推算值；此 commit 先顯示 effective 或佔位）。
- **「手動鎖定」**：填日期 → `task.start=` 該值，引擎當錨點、不覆蓋。
- 新增 `task.startMode`（`'auto'｜'manual'`），**純 UI 意圖記憶**；判定看 `startMode`、**不看 `start` 有無值**——因 `buildTaskFormHtml` 在編輯模式收到的 `start` 已被換成 `getEffectiveSchedule` 的 effective 值（會污染「有值＝手動」的判定）。
- 共用 `readStartField()`（讀雙態 → `{start, startMode}`），`saveNewTask`/`saveTask` 兩存檔點共用，單一真實來源。
- 舊任務 fallback（無 `startMode` 欄位）：`start` 有值當 `manual`、空當 `auto`，一次性相容。新任務一律預設 `auto`。
- **預計開始已放掉必填**（自動態無值可填）。
- 引擎（computeSchedule/applySchedule）**完全未動**；即時重算是下一段（2-A 後半，見 §10）。
- **commit：** `926f88c`（2-A 前半）。

---

## 第七部分：WBS 匯入器（現況，已查證）

`parseWbsExcel`(:6378) + `performWbsImport`(:6444) 已將 Excel 幾乎所有欄位寫入 task：

| Excel欄 | task 屬性 |
|---|---|
| A 序號 | wbs |
| B 階段 | stage |
| C 子群組 | subgroup |
| D 任務名 | name |
| E 類型 | taskType（+category lossy 過渡） |
| F 前置 | predecessor |
| G 工期 | durationDays |
| H 負責人 | owner + dept |
| I 預計開始 | plannedStart |
| J 預計完成 | plannedEnd |
| K/L 實際開始/完成 | actualStart/End |
| M 進度 | progress |
| N 狀態 | status |
| O/P 必交付/交付物 | mustDeliver/deliverable |
| Q 風險議題 | riskIssue |
| R 備註 | note |
| U/V 已交付/連結 | delivered/deliverableLink |
| 待補 | deadline（§6.6） |

**兩個 caveat：**
1. 匯入器刻意把 `start`/`end` 留空字串（只寫 planned），防 `getEffectiveSchedule` 誤判手填錨點。
2. **重新匯入整碗覆蓋**：先清空該專案任務再重建。匯入後本地編輯下次重匯被 Excel 覆蓋。工作流：來源是 Excel 的改 Excel 重匯；PM-Core 表單編輯適合手動新建任務。

H 欄解析 quirks：多人分隔符（`、` `/` `＋`/`+`）全要拆；髒值（`—`、`「負責人」`表頭）→ 未指派；未知名 → 未指派；H 欄可能直接是部門名 → 反查表要含部門名為 key。

---

## 第八部分：部門視圖（D 系列）

- **D-1 已完成**（`83cd433`）：parseWbsExcel 讀部門表 → 反查 map → task.dept（存名稱）；render 依 dept > subgroup > owner 分組。
- **D-2 結構定案**：`project.depts = [{id, name, members:[{id,name}]}]`，task.dept 存 dept **id**（非名稱，dept 改名不破壞關聯），`U.id()` 自動產 id。
  - D-2a：匯入存 project.depts（id 結構）。順序：存 depts → 建 members → 建 dept-id 反查 → translate task.dept 存 id。
  - D-2b：ownerToDept 由 project.depts 重建；render 加 fallback（dept id 找不到→未指派）；id→name 顯示。
  - D-2c：專案頁部門指派 UI（per-project，+/- dept、+/- member、改名 dept id 不變、刪 dept 提示 N 任務變未指派）。
  - D-2d：刪 dept 批次重派 modal（逐列下拉重派、重派後該列消失計數遞減、計數=0 才能刪、不離開情境）。
- **D-3（未來）**：task.dept → depts 陣列 + owners 陣列，一筆任務展開成多部門各自算負荷（解 D-1 單擔當灌爆 PM 負荷問題）。
- 部門 render fallback 標準：`hasVal ? ... : '未指派'`。

---

## 第九部分：導航架構（Sidebar 兩大區 + 分段視圖）（2026-06-07 定案）

> B 項（Sidebar 三區重構）的定案設計。動工前此節必須先 commit。
> 導航形態經議題討論定為「丙」：雙視圖不是全域 tab，而是各區內部各自切換視圖。

### 9.1 Sidebar 結構（兩大區 + 其他，共三段）

```
Sidebar
├─ 總儀表板區（彙總全專案）
│   ├─ 彙總 Dashboard   ← 視圖一（行事曆/時間軸）
│   ├─ 甘特圖           ← 全專案範圍
│   └─ 月曆視圖         ← 全專案範圍
├─ 專案區（兩層，不做樹狀展開）
│   ├─ J 系列壁掛分離式…  ← 點了進專案頁
│   ├─ 測試-排程驗證
│   ├─ 物料標準共用化
│   └─ ＋ 新增專案
└─ 其他（最下）
    ├─ 報告（專案週報 + PDCA）
    └─ 設定
```

### 9.2 視圖切換 = 分段鈕（非全域 tab）

- 切換用「分段按鈕」（參考第一版 WBS 的 `看板 / Gantt / 清單` 樣式），置於主畫面右上。
- 各區/各專案**內部**切換自己的視圖（導航形態「丙」），不是全域並列 tab。
- 總儀表板區分段鈕：`Dashboard ｜ Gantt ｜ 月曆`（全專案範圍）。
- 專案頁分段鈕：`Dashboard ｜ Gantt ｜ 月曆`（單專案範圍）。

### 9.3 雙視圖對應（接續第二部分雙視圖模型）

| 視圖 | 落地位置 | 內容 |
|---|---|---|
| 視圖一（時間軸） | 總儀表板區的「彙總 Dashboard」 | 個人週排程、智慧排程按鈕、會議設定、Task 可點編輯、Task 可拖動到時段 |
| 視圖二（進度/待辦） | 專案區的「專案 Dashboard」 | 階段進度 / 部門負荷 / 逾期清單（即現況 J 系列那頁） |

- **拖動規則**：在彙總 Dashboard 上拖動過的 Task，智慧排程不再覆蓋其時間（手動位置優先，呼應第四部分「手填日期不覆蓋」）。
- **計算層獨立**：部門負荷 / KPI 吃全部任務，不受視圖分流影響（沿用 §2.4）。

### 9.4 Gantt / 月曆共用元件原則

- 總儀表板的 Gantt/月曆 與 專案頁的 Gantt/月曆 **是同一套渲染元件**，差別只在傳入的 task 範圍（全專案 vs `task.project === pid`）。
- 禁止複製兩份（最高原則：抽共用、不重複）。動工前需撈現有 Gantt/月曆 render 函式，確認能否以「傳入 task 子集」方式重用，或需先解耦。
- 月曆的「預計/實際完成率」細項：順位低，後補（先前已討論，目前順位低）。

### 9.5 留白原則

- 未接好/未實作的視圖：分段鈕頁簽要在（讓缺口可見），頁面可留白（議題 4 定案）。

### 9.6 明確排除（不在 B 範圍，另案處理）

1. **專案模板分類（全新開發案 vs 設變案）**：獨立大功能，另開專案設計+執行。需設計
   `project.type` 資料結構、兩種 Dashboard 版面（全新=完整階段模板；設變=簡易模板）、
   建專案時選類型的流程。**不綁進 B**（避免導航重構膨脹成模板系統）。
2. **Sidebar 樹狀展開（專案 > 階段 > Task）**：不做。階段/Task 細節在專案頁內看
   （專案 Dashboard 的階段進度卡、Gantt 的展開列），不在 sidebar 展開。
   理由：與專案頁內視圖切換功能重疊＝重複，違反最高原則。

### 9.7 實作分段（待規劃，動工前此文件需先 commit）

> 實際拆法需先撈以下 code 才能定，列為下一步：
> - 現有 `showPage` 路由（app.js:1828 起）如何改成「區 + 視圖」二維。
> - 現有 6 導航項（總儀表板/甘特圖/月曆視圖/專案週報/PDCA報告/設定）如何搬進新結構。
> - Gantt（renderGantt 相關）/ 月曆（renderMonth 相關）元件如何改吃「傳入範圍」。
> - 分段鈕元件設計（新建共用元件，總儀表板與專案頁共用）。
> 每段獨立 commit，逐一核 diff → 線上驗證 → commit。

---

## 第十部分：待施工清單（依優先序排列，2026-06-08 重排）

每項獨立 commit，逐一核 diff → 線上驗證 → commit。

**已完成（導航架構落地，原 §10 第 7 項）**
- ✅ B-1 雙視圖頁內切換（修法二，`aa7a0d8`）：總儀表板頁頂分段鈕（儀表板｜甘特｜月曆），三視圖同頁互切。
- ✅ B-4 sidebar 三段重組 + 報告頁分段鈕（`724d6bd`）：sidebar 改三段（總儀表板/專案/其他）；報告頁頂分段鈕（專案週報｜PDCA）。
- ✅ B-2 專案頁接分段鈕（`9247b37`，待家裡桌機驗）：專案頁頂分段鈕（儀表板｜甘特圖｜月曆，單專案範圍）；renderGantt 加 singleProject 參數隱藏單專案篩選 chips；月曆頁簽留白待 B-3。

**第一優先：B 系列收尾（地基，低風險）**
1. ✅ B-3 `renderMonth` 月曆解耦（完成 `91086f8`）：單專案範圍的 task/meeting 篩選。B-2 月曆視圖需要。

**第二優先：核心大功能（要先出設計文件）**
2. 專案 Task By Stage 三視圖（看板/Gantt/清單，參考舊版 WBS）：依 stage 分組，三種呈現切換（看板拖卡 / Gantt 時間軸 / 清單表格）。依賴 B-2 分段鈕框架落地。動工前須先產獨立設計文件（三視圖呈現規格、By Stage 分組邏輯、與現有 renderGantt 共用方式）。

**第三優先：UI 微調（純 CSS，動工前出 mockup）**
3. 總儀表板分段鈕放大 + 配色：現 `.tab-btn` 太小不顯眼，放大並配色讓可點性明確。
4. 週報專案資訊欄對齊：每個專案的資訊欄未對齊，調版面。

**第四優先：整理（低優先）**
5. 設定頁 v2 重規劃：選項雜亂，盤點分類；側欄預設收合。

**Gantt 相關另案（B-2 衍生，依優先序）**
1. ✅ B-1 遺留（完成 `4c50190`）：renderGantt 無參數重繪丟 targetId/singleProject — toggleGanttProject/切週/拖曳呼叫 renderGantt() 不帶參數，重繪跑回 page-gantt，內嵌甘特畫面不更新。會壞功能，最優先。
2. 專案頁 Gantt 任務依賴連接線（FS/SS/FF/SF 箭頭線）— 大功能，需獨立規劃。
3. ✅ 總儀表板 Gantt 篩選 chips 改下拉選單（完成 `d20cf75` + `ed8fb3d`）— 小 UI 美化，碰 CSS 需先出 mockup，順位最後（除非順手可快速解決）。

**雙視圖原架構遺留項（§2 雙視圖模型相關，待雙視圖完整推進時處理）**
- 新增「排入行事曆」欄位 + 分流邏輯
- 視圖一（時間軸/時段制）呈現、視圖二（進度/待辦/逾期清單）呈現
- 部門負荷計算改用統一 H（工期×dailyHours 攤平到區間）
- 緊急任務清單移植（依賴視圖二定案）

**第 2 步：排程引擎接 UI（M2 表單 × 排程引擎）**
- ✅ 2-A 前半（雙態錨點 UI + 存檔 + 放掉必填）：`926f88c`。預計開始自動/手動雙態（方案甲，見 6.8），引擎未動。
- 2-A 後半（即時重算接線）待辦：前置/工期改 → debounce 300ms → `computeSchedule`（整專案）→ 取本任務結果填自動態顯示。決策：B 即時、full 連動、**絕不碰 `task.start`**。
- 2-B（錯誤通知）待辦：循環/斷鏈/無錨點 → 彈窗列清單 + 跳轉 + 反紅 + 逐筆引導。
- 2-C（清單拓撲排序）待辦：吃現成 `topoSortTasks`，依賴鏈視覺排在一起。

**表單/匯入器遺留項（原 A/B 群）**
- ✅ Tooltip 統一：依欄位說明統一規範（見 6.3）——單句用自製 data-tip 即時 hover（類型/緊急/狀態/HL，C 方案），複雜欄位（前置）維持常駐 `.field-hint` + 範例卡。原生 `title` 僅功能性標記保留。
- ✅ 前置任務結構化下拉（§6.4）：`71c7e78` + `ff3a3fd`。
- 移除 category 連動 → 行事曆配色改讀 taskType
- 逾期口徑統一（4 處改讀 `deadline || plannedEnd`）
- 匯入器補 deadline 欄、預計完成自動算

**架構整理遺留項（原 D 群）**
- app.js 拆檔（~7000 行）、部門 D-2c/D-2d/D-3

---

## 第十一部分：工作鐵則

**Git commit-gate（最高優先）**
- 嚴禁 `git add` 與 `commit` 串成一條（禁 `&&`/`;`）。
- commit 前單獨 `git status`，完整輸出給使用者，確認只有預期檔、無 local 機密檔（config.local.js/seed.local.js/seed.sample.js/_probe*.js）後，才 add → commit → push。
- 三步分開逐一核可。
- commit 訊息避 `>` `/`（PowerShell parser error），用兩段 `-m`。
- 未線上驗證標 `[unverified]`。

**跨機鐵則**
- 公司桌機（C:\Users\1141103004\，無 Node.js，CSS/UI）與家裡桌機（C:\Users\user\，有 Node.js）輪流。
- session 開始先 `git pull` + `git log` 確認與遠端同步，不一致先 pull。
- 換機/下班前未完成也要 commit + push（可標 WIP）。

**CSS 鐵則**
- 顏色/圓角/z-index/陰影一律走 :root 變數，禁規則裡寫死 hex/數字。
- 合理例外：rgba 透明衍生、膠囊 99px、圓點 50%。

**改檔紀律**
- 含中文檔用 Edit 工具，禁 PowerShell 文字回寫（UTF-8 變亂碼）。
- `?v=` 版本號只升動到的檔對應行，不全升。

**測試**
- Node.js（家裡桌機）跑 `node docs/test-schedule-cases.js`（56 案）+ `node --check app.js`。
- 測試檔含核心函式副本（parsePredecessors/isTaskBlocked/computeSchedule/topoSortTasks/isJTask/getJOverride/applySchedule/getEffectiveSchedule），改動兩邊同步。

**Claude Code**
- 確認對話框一律按 `1`（never allow-all）。
- 標準流程：邏輯寫完先 commit + 部署 + github.io 線上測，骨架能動再做 CSS。
- agent 兩壞習慣防範：①跑完自己往前衝（要它停、貼原文）②該貼原文只給摘要（明講貼 N 行原文）。
- 一次一件：寫一處 → 貼 diff 核 → 放行 → 下一處。

---

## 第十二部分：環境與資源

- **Runtime**：no-build、browser-native JS；`file://` 本地開發；Google Sign-In（擋 file:// 與 localhost，互動驗證走線上 github.io）。
- **Repo**：`PaulHsu02060/pm-core-paul`（Public，Pages 已上線）。
- **部署**：push 後約 1-2 分自動部署。
- **本地測試資料**：`seed.local.js`（gitignored）。
- **機密**：`config.local.js`（gitignored）。

---

## 第十三部分：物料模組

> 狀態：**規格草案、未動工**。本節只收已定案骨架，不搬 v9.3 逐欄公式細節。
> 完整探索細節見 `docs/material-module-exploration.md`。

### 13.1 定位

- **專案下的選配模組頁面**：非每案都有，僅「大型開發案 + 進入生產/組裝廠階段」才掛載。
- 需要**模組開關**概念：某些專案啟用物料模組、某些不啟用。
- 與「專案模板分類（全新開發案 vs 設變案）」相關，可能是「全新開發案模板」才有的分頁。
- 與現有功能正交：現有 PM-Core 管任務/排程/進度（時間維度），物料模組管料件/BOM/庫存/缺口（數量維度）。資料模型不同，**不可硬塞進 task 結構**，為專案頁的獨立分頁。

### 13.2 v9.3 計算鏈摘要

唯一輸入 → 需求 → 缺口 → 採購的單向鏈，另兩條平行分支：

```
唯一輸入：機台需求源（機型 × 階段台數）× 適用性矩陣（1/0）
   ↓ 相乘加總
各物料需求量
   − 庫存（性試後庫存 = 性試前庫存 − 性試耗用）
   ↓
整機缺口（待新組需求 − 性試後庫存，負值歸零）
   + 品保樣／封樣／商檢樣（另購、不耗庫存）
   ↓
採購小計
   ‖ 共用料競爭分配 → 實際可組幾台（外機可組分析）
   ‖ EMI 重洗例外（獨立計算，不併入主表）
```

最痛點是**共用料競爭分配**：N 個機型搶 M 個共用料，試算表只能手動試分配，程式可自動算最優分配。

### 13.3 資料結構：設計 A — 全域料件/庫存層

- **料件主檔與庫存放全域層（專案之上），跨專案共享一份。** 各專案引用全域料件、扣全域庫存。
- 理由：跨專案搶同一批庫存是真實場景，庫存掛專案下會打架（算誰的、扣誰的），全域層才乾淨，符合單一真實來源最高原則。
- **全域是資料層、不是顯示層**：料件「庫存 8 個」是客觀事實，跟從哪個專案看無關。顯示層仍分專案——專案頁傳入 `pid`，算「本案對全域料件的需求/佔用視角」（同 B-1 甘特 scope：同一份資料、傳 pid 篩視角）。
- 維護放全域層（一處改庫存全專案同步），需求/BOM 放專案層。

### 13.4 雙層值機制（核心骨幹）

繼承排程引擎 `getEffectiveSchedule`（override > computed、帶來源），套到物料每個數字：

- `computed`：公式算出，唯讀。
- `override`：人手改，**帶 reason + 時間**（解決 v9.3「改完不知為何改」的痛）。
- `effective = override ?? computed`：顯示用。
- 每格兩動作：①覆寫此格（手改、記原因）②還原公式（清 override、回吃 computed）。被手改的格有視覺標記 + hover 顯示原因。
- 結果不對有兩條路：①還原公式 ②**回源頭改輸入讓全鏈重算**（治本，呼應「源頭改對勝於中間補丁」）。

### 13.5 時間維度 — 採購/組裝 road map

核心需求，本模組相對試算表的最大價值。回推鏈：

```
組裝需求日（撈該階段組裝時間點）
   − 物料 L/T（前置交期 Lead Time）
   ↓ 回推
該物料「最慢下單日」= 組裝需求日 − L/T
```

- **與排程引擎同源**：「組裝日 − L/T = 最慢下單日」的日期回推 ≡ 排程引擎 addWorkdays / 前置 lag / 工作日曆；物料 L/T 本質是一種「前置 lag」（等料時間）。
- 組立時間點雙路徑（分層，同雙層值哲學）：**主路徑關聯 WBS task**（撈 `task.plannedStart`，task 改期則組裝日與下單日自動跟動）；**逃生口手填日期**（關聯不到時）。
- 資料結構預留：料件加 `leadTime` 欄、組立需求帶「關聯 `taskId`（主）/ 手填日期（逃生口）」。

### 13.6 可調分配

共用料不夠分時的分配規則**可切換**（需求多優先 / 交期急優先 / 手動指定…）。設計同 `applySchedule(tasks, scope)`：策略當參數傳入、演算法可插拔——`computeBuildable(materials, demands, strategy)`。

### 13.7 分層紀律 — 四個 core 純函式

核心純函式只回傳資料、不碰 DOM/Storage，UI 層 render 吃算好的資料：

- `computeMaterialDemand`（需求源 × BOM → 需求）
- `computeShortage`（需求 − 全域庫存 → 缺口/採購量）
- `computeBuildable`（共用料競爭分配 → 可組台數，strategy 可調）
- `computePurchaseRoadmap`（組裝日 − L/T → 最慢下單日，接排程引擎）

### 13.8 待釐清（K 系列完成後、正式設計前再定）

1. **模組開關綁法**：`project.modules=['material']` 還是綁專案模板類型（全新案 vs 設變案）？
2. **庫存來源**：手動維護，還是接公司 ERP/進銷存？
3. **變動追蹤**：BOM/台數變動要不要留歷史？override 已有原因紀錄，是否需更完整的變更日誌？
4. **輸入方式**：Excel 匯入（同 WBS 匯入器，可直接匯 v9.3 當初始）還是純表單手建？
5. **road map 呈現**：畫進甘特、獨立時間軸、還是清單？
6. **關鍵字撈 task 比對**：用 task 名稱、階段、還是另設對照？撈到多筆怎麼處理？

> 優先序：本模組在 K 系列三視圖之後。完整探索細節（五張表角色、痛點實例、資料實體清單、分期）見 `docs/material-module-exploration.md`。

---

## 第十四部分：專案頁四視圖 + 案別 variant（2026-06-09 定案）

取代舊「儀表板/甘特/月曆」三視圖。專案頁資訊架構重整 + 新增「案別 variant」資料層。
動工前此節必須先 commit。承接第二部分雙視圖模型、第八部分部門 D 系列 id 制哲學。

### 14.1 問題根源（UX）

舊版專案頁：Dashboard 內容（KPI／階段進度／部門負荷）寫死在主區，分段鈕飄在右上角、沒真的控制內容。後果：要看 Task 得越過整塊 Dashboard 往下捲，切視圖不直覺。

兩種使用情境被擠在同一塊垂直捲動頁面：

- **PM 綜觀**：看整體進度、部門負荷、逾期 —— 聚合資訊。
- **執行/追蹤（含 RD）**：看一張張 Task 卡、按狀態分欄 —— 個別 Task，看板對成員友善。

### 14.2 解法：四視圖平級（綜觀 / 看板 / 甘特 / 月曆）

主畫面分段鈕（膠囊式 segmented control，置標題列右側）切換四個平級互斥視圖，按了就地替換主內容區，不往下捲、不多一層分頁。

| 視圖 | 內容 | 對象 | 預設 |
|------|------|------|------|
| 綜觀 | 日期卡 + KPI + 階段進度 + 部門負荷 + Task 清單 | PM 綜觀 | ✅ 預設 |
| 看板 | 階段時間條 + 篩選列 + 五欄看板（依 status 分欄） | 執行/RD | |
| 甘特 | 精簡頂部 + 甘特（現有 renderGantt） | | |
| 月曆 | 精簡頂部 + 月曆（現有 renderMonth） | | |

需求分級（使用者定）：綜觀（Dashboard + Task 清單）> 看板 = 甘特 > 月曆。

- **Task 清單** 併進「綜觀」頁（清單緊湊、適合配 Dashboard），不獨立成視圖。
- 對應架構演化：原 §9 的 儀表板/甘特/月曆 三平級 → 綜觀/看板/甘特/月曆 四平級（綜觀＝原儀表板＋清單，新增看板）。

接入點（已查證，乾淨）：

- `switchProjectView(view)`（現有 dashboard/gantt/month）→ 加 `kanban` case。
- `buildProjectViewTabsHtml()` → 多一顆「看板」鈕。
- 新增 `renderKanban()`：讀 `task.status` 分五欄（未開始/進行中/已完成/延遲/暫停）。分欄靠 status，不碰 stage 字串格式，安全。
- 不動 `showPage`/`renderPage` 路由骨架。

### 14.3 看板頂部「階段時間條」

- 資料直接吃現成 [CORE] 函式 `App.getProjectStages(projectId)`（:5447），已回傳每階段 `earliestStart` / `latestEnd` / `itemCount` / `doneCount` / `stageId` / `name` / `group`。不新增計算。
- 呈現：圓角卡片格式（沿用系統現有卡片 CSS class，不另寫新樣式 —— 抽共用、不重複）。每階段一張小卡（階段名 + 起訖年/月/日），排成一列；不用無縫橫帶。
- 標出「現在進行中」階段。
- 起訖走 `getEffectiveSchedule`（override > actual > scheduled > planned），與全系統一致。

### 14.4 案別 variant（兩列時間條的資料根基）

#### 14.4.1 問題

J 系列同一專案含主案（2.9~7.3kW）與另案（2.2kW），階段名重複（手工機 / 手工機(2.2)）。舊 code 靠 regex `isAlt = (nm) => /\([\d.]+\)\s*$/.test(nm)` 判別「階段名結尾帶括號數字」＝另案 —— 脆弱（全形括號、漏打、格式不一即誤判），且 code 自註「等真實 Sheet 核對後再定」，未驗證。靠字串長相猜「屬於哪個案」違反最高原則。

#### 14.4.2 解法：顯式欄位（全 id 制，同構部門 D 系列）

```js
project.variants = [{ id, name }]      // 例：[{id:'v_a3f', name:'2.2kW 另案'}]
task.variant = id                       // 存 id，不存名稱
空值/未填 = 主案                         // 永不賦 id（同「未指派」存 literal 哲學）
U.id() 產 id（與部門同一套）
```

與部門完全同構：`project.depts` ↔ `project.variants`、`task.dept` ↔ `task.variant`、「未指派 ↔ 主案」皆不賦 id 的空值/literal。部門那套防呆邏輯可直接複用，不重寫。

理由：案別名幾乎不改，但 id 制保險、避免資料錯亂、與部門一致。

#### 14.4.3 匯入器（回家做，碰資料）

- Excel WBS 模板多一欄「案別」。PM 填名稱（如「2.2kW 另案」），不填 id。
- 兩段式（同 D-2a 部門匯入順序）：
  1. 掃所有 task 案別欄，收集不重複名稱。
  2. 為每名稱建 `{id, name}` 存 `project.variants` + 建「名稱→id」反查。
  3. translate 每筆 task 案別名 → id 存 `task.variant`。
  4. 空白案別 → `task.variant` 留空（主案）。
- 同名複用 id（反查表去重，不可每筆各建一個 id）。

#### 14.4.4 顯示層（getProjectStages 分組改動）

- 廢除 `isAlt` regex。
- 改讀 `task.variant`：有 variant id → 歸該另案；空值 → 主案。
- 時間條按 variant 分列：主案上列；各另案各自一列在下。每列左側標案別名（主案＝一般色、另案＝警示色區隔）。
- 無另案的專案：自動只顯示主案一列（不留空、不浪費高度）。

#### 14.4.5 fallback 防呆（沿用部門標準）

- render 找不到 variant id → 當主案顯示，不報錯。
- 顯示時 id→name 反查。

### 14.5 工作分層（公司純顯示層 vs 回家碰資料）

| 工作 | 性質 | 在哪做 |
|------|------|--------|
| 四視圖切換 UI（tab 加「看板」、switchProjectView 加 kanban case） | 純顯示層，接現成機制 | 公司可做 |
| renderKanban 看板本體（讀 status 分五欄） | 純顯示層讀 DATA | 公司可做 |
| 階段時間條（吃 getProjectStages 現成輸出、圓角卡片） | 純顯示層 | 公司可做（先做單列） |
| 新增 project.variants / task.variant 欄位 | 資料結構 | 回家做 |
| 匯入器補讀「案別」欄 + 兩段式 translate | 碰匯入器，需真 Sheet | 回家做（跑 56 案） |
| getProjectStages 改讀 variant（廢 isAlt）+ 兩列時間條 | 碰計算層分組 | 回家做 |

### 14.6 過渡狀態（已知、非 bug）

廢 `isAlt` 改讀 `task.variant` 後，尚無 variant 欄位的現有資料全歸主案 —— J 系列 2.2 另案在 Excel 補案別欄並重匯前，會暫時全顯示為主案一列。Excel 補欄重匯後 2.2 才正確分到下列。此為正常過渡。

### 14.7 明確排除 / 順位低

- **Task 清單** 視圖：併入綜觀頁，不獨立。
- 主/副案以外的多重另案（R4 等）：variant 結構天然支援（多個 id 多列），但呈現待真實多另案資料出現再驗。
- 月曆視圖順位最低（需求分級最末）。

---

## 附錄：今日（2026-06-06）完成的 commit

- `8f0544a` 移除分類/處理方式欄（UI 拿掉，category 資料層保留）
- `f70a2c0` 欄位改名排序加必填驗證
- `e40931c` HL 風險勾選 + 交付物 + 實際執行反向摺疊
- `5eaa1f9` docs：M2 任務表單設計文件
- 基準 HEAD：`5eaa1f9`，引擎 56/56 PASS

## 附錄：今日（2026-06-08）完成的 commit
- 91086f8 B-3 renderMonth 月曆解耦：加 monthScope，修換月/換年重繪丟範圍跳回總月曆
- 4c50190 B-1 renderGantt scope 修復：加 ganttScope，修切週/切篩選重繪丟範圍跳回總甘特
- d20cf75 甘特專案篩選 chips 改多選下拉 JS 骨架（ganttFilterOpen 維持展開不被重繪沖掉）
- ed8fb3d 甘特篩選下拉 CSS 樣式 + style.css 版本號 20260608-5
- 198450d 專案頁標題垂直置中（proj-name 移除吊空 margin）
- 097c206 專案頁右欄部門負荷/逾期字級對齊左欄 + style.css 版本號 20260608-6
- 全部 [unverified] 待實機驗證
- 資料：6/1 週報 4 小專案 6 task 經 Console 注入線上（J系列未動）
- d2c8e9e docs：物料模組第十三部分（docs/.claude 雙份 + 探索文件入版控）
- 71c7e78 前置任務結構化下拉（取代自由文字，serializePredecessors 共用序列化，格式 16FS/16FS+2 引擎相容）
- ff3a3fd 前置任務 UI 重設計（彩色 label + 整句白話關係下拉 + 緩衝常駐 + 範例卡）+ 類型/緊急/狀態 `?` 改常駐 field-hint 小字
- 926f88c 預計開始自動/手動雙態錨點（方案甲，§6.8；新增 startMode + readStartField 共用、放掉必填，引擎未動）
- （本次 commit）欄位說明改自製 data-tip 即時 hover（類型/緊急/狀態/HL，C 方案；前置維持常駐 field-hint + 範例卡）+ app.js?v=20260609-1。
- 上列 5 筆 [unverified] WIP，線上 UI 待驗。
