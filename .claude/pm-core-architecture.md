...
> 最後更新：2026-06-20（家裡桌機，基準 HEAD `c16f940`）
...
## 第八部分之七：共用表格規範（`.data-table`）（2026-06-20 定案）

> 全系統「資料表格」單一寬度真實來源。動工前此節必須先 commit。
> 治本目標：不單獨調每個表格欄寬，抽一套規範，改一處全動、新表格零微調。

### 8g.1 路線定案：統一 `<table>`（路線①）

盤點全系統 8 個資料表格：6 個已是 `<table>`、2 個是 div-grid（待辦 `.task-grid`、公休日 `.cal-row`）。

三條路擇一：
- 路線①（定案）：全部統一成 `<table>`。`<table>` 原生跨列對齊欄、`table-layout` 直接給「內容自動寬 + 靠左」，6/8 已就位，只需移植 2 個 grid 表。**唯一真正「一份邏輯」解，符合最高原則。**
- 路線②（否決）：共用欄類型語彙、grid 與 table 兩套機制各自實現。改 token 兩邊都變，但機制仍是兩份——與「單一真實來源」抵觸。
- 路線③（否決）：全部改 div-grid + subgrid。遷移量最大、把 grid 欄寬坑複製到全系統。

subgrid 是 grid 專屬、`<table>` 套不到，故無法當共用基底——這是排除路線②③的關鍵事實。待辦表格的 subgrid（`dcae482`）為沉沒成本，不綁架方向。

### 8g.2 兩層機制（寬度與行為分離）

- **寬度**：由 `<colgroup>` + `table-layout: fixed` 決定。這是 `<table>` 原生能力、div-grid 沒有的，也是治本核心。寬度數值全走 `:root` token。
- **行為（對齊／截斷）**：由欄類型 class 決定，不含寬度。

兩者分工＝乾淨的單一真實來源：改 token 一處，全系統同步；新表格只排 colgroup + 標 class，零微調。

▎ 欄類型 class 兩用且分離：`<col class>` 給寬度（`<col>` 只認 `width`）、`<td>`/`<th>` class 給對齊/截斷（`text-align`/`ellipsis` 須掛 cell，`<col>` 不認）。`.col-flex` 不掛 col 寬、吃剩餘空間。

### 8g.3 `:root` 寬度 token（CSS 鐵則）

| token | 用途 | 套用欄 |
|---|---|---|
| `--col-num-w` | 短固定欄 | 序/#、狀態、進度%、餘裕、截止、工期、燈號、週次、短日期(M/D) |
| `--col-mid-w` | 中欄 | 階段、負責人/擔當、前置、類型、需交付、時程區間、預計開始/完成、專案/議題 |
| `--col-action-w` | 操作欄 | 刪除鈕、操作鈕 |
| （無 token） | 彈性主欄 | 任務名、大項目、本週工作/狀況、補回計畫、備註、描述（吃剩餘空間，不給固定寬） |

寬度單位用 `rem`（跟字級走、改字級不破版；不用 ch，避免中英混排跨列對齊歪）。

▎ 完整 ISO 日期（`2026-01-01`）寬度超過 `--col-num-w`，歸 `--col-mid-w`；`--col-num-w` 只適合 M/D 短格。

▎ token 寬為寬表基準；窄 modal 表（540 modal、6 欄）固定欄合計超出可用寬時主欄被壓縮 → 已定案 `.data-table.compact` 變體覆寫窄值，見 §8g.3b。


### 8g.3b 窄 modal 變體 `.data-table.compact`

寬表 token（num 4rem／mid 8rem）是寬容器基準；540 modal 內 6 欄表（Excel 匯入預覽、任務史）固定欄合計 448px 超過可用寬 ≈498px，主欄只剩 50px。解法＝窄容器 token 變體，不改 col 規則：
- 機制：CSS 自訂變數從 `<table>` 繼承到 `<col>` 子層，故 `.data-table.compact { --col-num-w; --col-mid-w }` scoped 覆寫即生效，`col.col-*` 規則一字不動——同一組 var、只在窄容器覆寫，**單一真實來源不破**。
- 暫定值：num 3rem（42px）／mid 6rem（84px）。算式 2×42＋3×84＝336，主欄拿 498−336≈162px。rem 實值最後在瀏覽器肉眼微調（ISO 日期欄需 ≥80px、勿被截）。
- 套用：540 modal 內 6 欄表掛 `class="data-table compact"`；寬表（總板／專案頁）維持標準 token、不加 compact。

### 8g.4 欄類型語彙（4 類，只管行為）

- `.col-num`：短固定欄，置中、不截斷。
- `.col-mid`：中欄，靠左、`ellipsis` 截斷。
- `.col-flex`：彈性主欄，靠左、`ellipsis` 截斷，吃剩餘空間（每表通常一欄）。
- `.col-action`：操作欄，置中、固定窄。

▎ 多行例外 `.col-wrap`（修飾，**非第 5 欄類型**）：col-mid/col-flex 預設 `nowrap`+`ellipsis` 會砍多行 cell。需保留多行的 `<td>`（如任務史「預計完成」帶 `原:延期前日` 第二行）改標 `.col-wrap`（`white-space:normal`、不截斷）。與 4 欄類型正交：col-* 給對齊基準、col-wrap 解除截斷，可疊加。需在 .data-table 補一條 CSS。

### 8g.5 截斷溢出 hover 全文

`.col-flex` / `.col-mid` 截斷後，render 時 cell 給 `title="{全文}"` hover 顯示全文。沿用待辦前置欄既有手法，全系統一致。

### 8g.6 遷移順序（先易後難，逐步獨立 commit）

每步獨立 commit，逐一核 diff → 線上驗證 → commit。

| 步 | 對象 | 動到的檔 | 風險 | 機器 |
|---|---|---|---|---|
| 1 | 建 token + `.data-table` + 4 class | style.css | 純 CSS 機械 | 任一機 |
| 1.5 | 補 colgroup 寬度 CSS（`col.col-*` 接 token） | style.css | 純 CSS 機械 | 任一機 |
| 2 | 試金石：WBS 匯入預覽（單表 5 欄，純 inline→class、零結構、540 modal 裝得下） | app.js render | 低 | 任一機 |
| 3 | 其餘扁平 `<table>`：Excel 匯入預覽（6 欄、modal 偏擠需調欄）/任務史 | 各 render | 低-中 | 任一機 |
| 4 | `.cal-row`（年份分組 → 每年 `<tbody>` + colspan 年份列 + sticky） | app.js render + css | 中 | 任一機 |
| 5 | stage2 `.s2-tbl` | render | 中 | 任一機 |
| 6 | 待辦 `.task-grid` div-grid → `<table>`（互動移植：列點開/列間➕/hover/done-undated 摺疊） | app.js 大改 | 高 | 家裡桌機 |

步1–4 不動核心邏輯，`node --check` 即可；步5 是唯一判斷風險改動，隔離成獨立 commit、家裡桌機跑驗證。

### 8g.7 明確排除（不套此規範）

圖表/日曆/卡片結構特殊，不套通用表格規範：甘特 `.gantt`、週曆 `.week-schedule`、月曆 `.month-grid`、看板 `.kanban-board`、KPI 卡 `.report-summary`。
另：週報表 `.rp-table` 是固定 8 欄列印文件（淡色列印皮膚、格式穩定、無「改欄名手調寬」痛點），排除規範、保留自有 `.rp-table` 設計。
另：PDCA 月報 `.pr-group-table`（`exportPdcaReport` 匯出的獨立單頁 HTML，自帶 `pdcaReportStyle`、Blob URL 開新分頁）排除規範——它不吃 app 的 `style.css`，`.data-table`（定義在 style.css）到不了它，性質同 `.rp-table` 列印文件，保留自有列印樣式。

### 8g.8 死碼清除（步5 同批或後續）

待辦表格遷 `<table>` 後，清 subgrid 殘留：舊 fr 規則、`#activeTaskList` 系選擇器、`.toschedule-group.collapsed`。
