...
### 4.8 兩套排程引擎的分工（2026-06-12 定案，最高優先釐清）

> 此節是「為什麼有兩套排程、各管什麼、哪套要按鈕哪套自動」的單一真實來源。
> 反覆被誤解，每換 session 都要重講——以此節為準，不再口頭解釋。

系統有兩套本質不同的排程引擎，對應第二部分雙視圖模型。它們不是重複、不該合流，各管一種任務：

- 時段制週排程（generateSchedule，generateNow/generateGlobalSchedule 呼叫）：管小時制工作項目（很多小細項、會議），把它們排進這週合適的時段格子。手動按鈕（總儀表板「智慧排程」）——時段排程本質需要人工觸發，這顆按鈕保留。對應視圖一。
- 工期依賴排程（computeSchedule/applySchedule，FS/SS/FF/SF、105 測試覆蓋）：管 WBS 工期任務，算前置鏈、自動傳播每項開始/結束日。不該有按鈕——資料輸入完就該自動算。對應視圖二。

核心原則：工期依賴排程是「資料輸入完即自動算」，不是手動觸發。「自動排程要人按按鈕」本身就是接線斷掉的症狀。正確設計是存檔即驅動：
- saveNewTask/saveTask 存檔後自動觸發 applySchedule（工期依賴引擎），前置鏈自動傳播算出 scheduledStart/End。
- 甘特圖「⚡一鍵套用排程」按鈕（applyGanttSchedule）廢除——存檔自動觸發後它多餘，它的存在就是引擎沒接的證據。過渡期可先留作對照組驗證，驗穩後移除。
- 總儀表板「智慧排程」按鈕保留——那是另一套時段制週排程，與工期依賴自動排是兩回事，不可一起砍。

手填日期保護（自動觸發的風險點）：存檔即觸發會讓「尊重手填不覆蓋」的判定每次存檔都跑，比手動按一次頻繁得多。computeSchedule 必須確實尊重手填錨點不覆蓋，自動觸發放大任何判定漏洞，故為判斷風險最高的改動。

### 4.9 正推 / 反推雙模式（2026-06-12 定案，反推待新增）

使用者只填預計開始與預計結束，提供選項切換排程基準（呼應 §N.3）：
- 正推（forward，依預計開始日順排）：從開始日往後按工期+依賴推。引擎已有（computeSchedule）。
- 反推（backward，依結束日反推）：從交期往前倒推，算每項最晚必須何時開始/完成，否則跳票。引擎尚無此算法，需新增 backward pass。核心函式改動、判斷風險最高、需家裡桌機 Node 跑回歸測試。

理由：專案時間跨度不同——跨度大時，有人要從 deadline 倒推卡點，有人要從起點順排。

路線決策（沿用 §N.6）：先做正推版，反推 UI 留位子標未開放、灰掉不可選，等引擎補反推算法再點亮。反推算法列為工期引擎自動觸發（§4.8）穩定後的下一階段。
