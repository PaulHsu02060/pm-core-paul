...
**段4（UI 顯示，低風險）— 版號欄位 + sidebar 顯示**：project 加 version/importedAt（巢狀欄位自動持久化，無四步/無 download 防坑顧慮）、sidebar 同名專案顯示版號 + 日期（純 UI 層）。線上驗：sidebar V1/V2 + 日期正確、跨裝置不掉欄位。
...
### 16.7 施工分塊
塊1 :root 新增 --nav-active/--danger/-d/--ink-btn（純加變數）
塊2 .tb-action.ink variant + .tb-action.danger 改引 --danger（CSS）
塊3 .view-tabs-bar .tab-btn.active scoped --nav-active（CSS scoped）
塊4 buildProjectHeaderHtml 重排（HTML）
塊5 ⋯ 更多選單 + 匯出下拉元件（HTML+CSS+JS toggle，z-index --z-dropdown、點外關閉）
