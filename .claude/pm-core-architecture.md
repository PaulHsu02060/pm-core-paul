...
8. 視圖一（時間軸/時段制）呈現
9. 視圖二（進度/待辦/逾期清單）呈現
10. 部門負荷計算改用統一 H（工期×dailyHours 攤平到區間）
11. 緊急任務清單移植（專案頁=該專案、總儀表板=全部，可點看細節）—— 依賴視圖二定案

**D 群：架構整理（低優先，feature 穩定後）**
12. app.js 拆檔（~7000 行，no-build 約束，ES modules 或 ordered script）
13. 設定頁 v2（移除 J 系列遺留、側欄預設收合）
14. 部門 D-2c/D-2d、D-3
