...
> 由來：J 系列 Excel 日期由前置 WORKDAY 推算、排除「冰點公休」，但 PM-Core 假日表
> 未對齊冰點行事曆 → 現有 55/93 筆日期不一致。工作日曆是讓引擎日期與 Excel 一致的地基。

### 之二.1 驗收標準（Excel 已給）
- 引擎算出 2.9~7.3kW 可販日(#60) = 2027/1/29
- 引擎算出 2.2kW 可販日(#123) = 2027/3/31
- 對齊行事曆後，引擎日期應與 Excel 預計開始/結束一致、55 筆差異收斂
...
3. 匯入器：Excel 冰點行事曆 → base（PDF/截圖後補）。
4. 引擎接 isWorkday（核心改動，逐 diff + 105 測試 + 補日曆測試）。
5. 設定頁 UI（先 mockup 審）。
6. 驗收：重算 J 系列，#60=2027/1/29、#123=2027/3/31、55 筆差異歸零。

### 之二.8 先做哪步（建議）
先做 2+3（isWorkday + 匯入冰點行事曆 28 個公休日），重算對照 Excel。若 55 筆差異大幅
收斂 → 證實主因是行事曆。UI（第 5 步）後補。

### 之二.9 匯入功能第一版（貼上文字解析，2026-06-13 定案）

第一版採「貼上 Excel 文字解析」，零外部庫（no-build 友善），不解析 .xlsx 檔本體（SheetJS 等留待第二版增強）。

**使用流程：** 設定頁「工作日曆」區 → 從 Excel 冰點行事曆框選整段複製貼進 textarea → 「解析並匯入」→ 預覽抓到幾筆 → 確認寫入 base.holidays + 持久化 → 下方顯示已載入公休清單（年份分組、單筆增刪）。

**解析規則**（對齊冰點行事曆欄位：日期/星期/類型/節日名稱/工作日(1/0)/備註，Tab 分隔）：
- 逐行切 Tab，取 日期(1)、類型(3)、節日名稱(4)、工作日(5)
- 類型==='公休日' → base.holidays[日期]=節日名
- 類型==='補班' 或（星期六/日 且 工作日===1）→ override.workOverrides[日期]=名（冰點目前無補班，保留供他公司）
...
**施工分段（每段獨立 commit）：**
1. 解析純函式 parseCalendarPaste(text) → {holidays, workOverrides, skipped}（純函式不碰 DOM/Storage）
2. 解析測試：餵冰點貼上樣本，斷言抓出 28 筆公休、日期名稱正確
3. 設定頁 UI（textarea + 解析按鈕 + 預覽 + 清單增刪，先 mockup 審）
4. 持久化接線（寫 DATA.calendars + Storage.save + 確認雲端帶 calendars）
5. 驗收：貼入冰點 → 28 筆進系統 → 重匯最新 Excel → 重算 → 不一致=0、#54=2027-01-28、#120=2027-03-22

**先做 1+2（解析純函式+測試），零 UI 零風險、可立即驗證；UI（3）先 mockup 審後做。**
