...
> 未對齊公司行事曆 → 現有 55/93 筆日期不一致。工作日曆是讓引擎日期與 Excel 一致的地基。
...
### 之二.8 先做哪步（建議）
先做 2+3（isWorkday + 匯入公司行事曆 28 個公休日），重算對照 Excel。若 55 筆差異大幅
...
2. 解析測試：餵公司行事曆貼上樣本，斷言抓出 28 筆公休、日期名稱正確
3. 設定頁 UI（textarea + 解析按鈕 + 預覽 + 清單增刪，先 mockup 審）
4. 持久化接線（寫 DATA.calendars + Storage.save + 確認雲端帶 calendars）
5. 驗收：貼入公司行事曆 → 28 筆進系統 → 重匯最新 Excel → 重算 → 不一致=0、#54=2027-01-28、#120=2027-03-22
